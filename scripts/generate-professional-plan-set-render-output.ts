...
async function generatePreviewAssets(dir: string, pkg: ReturnType<typeof buildProfessionalPlanSetRenderPackage>) {
  ...
  const contactHeader = Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="118"><rect width="1200" height="118" fill="#111827"/><text x="32" y="46" font-family="Arial" font-size="28" font-weight="900" fill="#ffffff">SolarPro Commercial Plan-Set Preview</text><text x="32" y="76" font-family="Arial" font-size="14" font-weight="700" fill="#d1d5db">A-101 evidence-derived photo-aligned reconstruction · explicit fallback labels · deterministic preview-only SVG/PDF package</text><text x="930" y="46" font-family="Arial" font-size="16" font-weight="900" fill="#fbbf24">QUALITY ${pkg.summary.renderQualityScore}/100</text><text x="930" y="74" font-family="Arial" font-size="12" font-weight="700" fill="#e5e7eb">${pkg.summary.renderQualityGrade}</text></svg>`);
...
- Flagship A-101 now prioritizes real survey-photo evidence and derived photo-aligned spatial candidates before existing geometry or clearly labeled synthetic fallback layers. Ground-mount and solar-fence visuals remain preview aids only when tied to evidence/fallback disclosure.
- Site plan viewport includes evidence-alignment summary, photo-derived roof/install/equipment/conduit/module cues, fallback disclosures, line-weight hierarchy, construction notes, revision/QA table, and non-authoritative review labels.
...
Average evidence-alignment render quality score: **${summaryValue.averageRenderQualityScore}/100**. Checklist keys: ${summaryValue.renderQualityChecklistKeys.join(', ')}.`;
}
