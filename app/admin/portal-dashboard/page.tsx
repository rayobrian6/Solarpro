          ...
          return (
            <div key={s}>
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                {!isCurrent ? <span className="text-[9px] text-emerald-500/40 ml-1">✓ Complete</span> : null}
            ...
            {filtered.slice(0, 20).map(p => {
              ...
              return (
                <button key={p.id} onClick={() => { setSelected(p); setShowPicker(false); setSearch(''); }}
                  ...
                  {p.address ? <p className="text-xs text-slate-500 mt-0.5 truncate">{p.address}</p> : null}
                  {c ? (
