...
export function calcStringSystem(input: StringSystemInput): StringSystemResult {
  ...
  for (const inv of input.inverters) {
    ...
    for (const str of inv.strings) {
      ...
      // ── Voc check (NEC 690.7) ────────────────────────────────────────────
      // v47.412 — Topology-aware Voc gate.
      // Optimizer systems regulate string voltage at the inverter bus
      // (~380–400 V for SolarEdge HD-Wave) irrespective of panel count, so
      // NEC 690.7 panel-Voc × N math is inapplicable. Brand-spec
      // maxPanelsPerString (25 for SolarEdge) enforces the real ceiling.
      if (inv.type !== 'optimizer' && vocCorrected > inv.maxDcVoltage) {
        issues.push({
