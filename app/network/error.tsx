...
export default function NetworkError({ error, reset }: { error: Error; reset: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] p-8">
      <AlertTriangle size={48} className="text-red-400 mb-4" />
      <h2 className="text-xl font-semibold text-red-400 mb-2">Network Error</h2>
      <p className="text-sm text-slate-400 mb-6 text-center max-w-md">
        {error.message || 'Something went wrong loading the network page.'}
      </p>
      <div className="flex gap-3">
        ...
        >
          <Home size={14} className="inline mr-1.5" />Network Home
        </a>
      </div>
    </div>
  );
