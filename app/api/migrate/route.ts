...
export async function POST(req: NextRequest) {
  try {
    ...
    // ══════════════════════════════════════════════════════════════════════════════
    try {
      // Each entry: name, state, net_metering, interconnection_limit_kw, buyback_rate, rate_structure, notes
      // buyback_rate = $/kWh (NEM retail, NEM3 export, or avoided-cost value)
      // rate_structure: "TOU" | "Tiered" | "Flat" | "TOU-Tiered" | "Flat/TOU opt-in"
      const utilityFullSeeds: Array<{
        ...
        { name: 'Rockland Electric',           state: 'NJ', nm: true,  limit_kw: 2000,  buyback: 0.195, structure: 'TOU',             notes: 'NEM retail; part of O&R; TOU residential' },
        // Pennsylvania
            ...
            await sql`
              ...
              VALUES
                (${u.name}, ${u.state}, 'US', ${u.nm},
                 ${u.limit_kw}, ${u.buyback}, ${u.structure}, ${u.notes}, 'seeded_022')
              ON CONFLICT DO NOTHING
            `;
            m022Count++;
          }
