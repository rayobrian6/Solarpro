...
export async function POST(req: NextRequest) {
  try {
    ...
    if (users.length === 0) {
      ...
      console.debug('[password-reset] Request for unknown email — returning success silently');
      return NextResponse.json(SUCCESS_RESPONSE);
    }
    ...
    // NOTE: The DB table has both a legacy `token` column (NOT NULL) and a newer
    // `token_hash` column. We store the hash in both to satisfy the NOT NULL constraint.
    ...
    console.error('[password-reset] request-password-reset unexpected error:', (error as Error)?.message, (error as Error)?.stack);
