...
export async function GET(req: NextRequest) {
    ...
    try {
      ...
      if (rows.length === 0) {
        ...
        try {
          ...
          results.password = { error: (e as Error).message };
        }
      }
    } catch (e: unknown) {
      results.user = { error: (e as Error).message };
    }

  } catch (e: unknown) {
    results.db = { connected: false, error: (e as Error).message };
  }
  ...
  try {
    ...
    results.jwt = { working: false, error: (e as Error).message };
  }
