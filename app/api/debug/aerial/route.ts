...
export async function GET(req: NextRequest) {
  ...
  if ((!finalLat || !finalLng) && address) {
    try {
      ...
      results.geocode = { error: e instanceof Error ? (e as Error).message : String(e), duration_ms: Date.now() - geocodeStart };
    }
  } else {
  ...
  if (finalLat && finalLng) {
    try {
      ...
      results.solar_api = { error: e instanceof Error ? (e as Error).message : String(e), duration_ms: Date.now() - solarStart };
    }
  } else {
  ...
  if (finalLat && finalLng) {
    try {
      ...
      results.satellite_image = { error: e instanceof Error ? (e as Error).message : String(e), duration_ms: Date.now() - staticStart };
    }
  } else {
