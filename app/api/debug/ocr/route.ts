  ...
  try {
    ...
    if (isPdf) {
      ...
      try {
        ...
        const msg = pdfErr instanceof Error ? (pdfErr as Error).message : String(pdfErr);
      ...
      if (ocrText.length < 50) {
        ...
        try {
          ...
          const msg = ppErr instanceof Error ? (ppErr as Error).message : String(ppErr);
      ...
      try {
        ...
        const msg = cliErr instanceof Error ? (cliErr as Error).message : String(cliErr);
      ...
      if (ocrText.length < 20 && process.env.OPENAI_API_KEY) {
        ...
        try {
          ...
          const msg = vErr instanceof Error ? (vErr as Error).message : String(vErr);
