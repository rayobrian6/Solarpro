...
async function runOcr(buf: Buffer, mime: string, name: string): Promise<{
  ...
  if (mime === 'application/pdf' || name.toLowerCase().endsWith('.pdf')) {
    ...
    try {
      ...
      try {
        ...
        debugLog.push(`pdftotext failed: ${(e as Error).message}`);
        try { fs.unlinkSync(tmpIn); } catch {}
      }
    } catch (e: unknown) {
      debugLog.push(`pdftotext setup error: ${(e as Error).message}`);
    }
    ...
    try {
      ...
      debugLog.push(`pdf-parse failed: ${(e as Error).message}`);
    }
  ...
  try {
    ...
    try {
      ...
      debugLog.push(`tesseract-cli failed: ${(e as Error).message}`);
      try { fs.unlinkSync(tmpIn); } catch {}
    }
  } catch (e: unknown) {
    debugLog.push(`tesseract-cli setup error: ${(e as Error).message}`);
  }
  ...
  if (openaiKey) {
    try {
      ...
      debugLog.push(`openai-vision error: ${(e as Error).message}`);
    }
  } else {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    trace.stages.reception = { ok: false, error: (e as Error).message };
    return NextResponse.json({ success: false, error: `File reception failed: ${(e as Error).message}`, trace }, { status: 400 });
  }
  ...
  try {
    ...
    trace.stages.ocr = { ok: false, error: (e as Error).message };
    console.warn(`[DEBUG_BILL_STAGE2] OCR threw: ${(e as Error).message}`);
  }
  ...
  try {
    ...
    trace.stages.parse = { ok: false, error: (e as Error).message };
    console.warn(`[DEBUG_BILL_STAGE3] parseBill threw: ${(e as Error).message}`);
  }
  ...
  if (parsedFields < 3 && ocrText.length > 50) {
    try {
      ...
      trace.stages.ai_fallback = { ok: false, triggered: true, error: (e as Error).message };
      console.warn(`[DEBUG_BILL_STAGE4] AI fallback threw: ${(e as Error).message}`);
    }
  } else {
    trace.stages.ai_fallback = {
      ok: true,
  ...
  try {
    ...
    trace.stages.rate_validation = { ok: false, error: (e as Error).message };
  }
