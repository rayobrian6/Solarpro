...
function defaults(type: OpportunityType, quality: LeadQuality) {
  const base = { monthly_bill: type === 'service_call' ? 90 : 260, estimated_project_value: type === 'battery' ? 42000 : type === 'roofing' ? 28000 : type === 'service_call' ? 1800 : 36000, battery_interest: type === 'battery', roof_age_years: type === 'roofing' ? 24 : 8, structure_type: type === 'service_call' ? 'single_family' : 'single_family' }
...
async function scoreAndPersist(sql: Awaited<ReturnType<typeof getDbReady>>, opportunityId: string, adminId: string) {
  ...
  const scored = scoreOpportunity({ monthly_bill: opp.monthly_bill as number, state: (opp.state ?? opp.location_state) as string, roof_age_years: opp.roof_age_years as number, structure_type: opp.structure_type as string, usable_roof_pct: (opp.usable_roof_pct ?? opp.roof_usable_pct) as number, stories: opp.stories as number, source_type: opp.source_type as string, estimated_system_size_kw: opp.estimated_system_size_kw as number, annual_usage_kwh: opp.annual_usage_kwh as number, battery_interest: (opp.battery_interest ?? opp.battery_candidate) as boolean })
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (['screen', 'score', 'release', 'match'].includes(action)) {
      if (!opportunityId) return jsonError('opportunity_id is required', 400)
      if (action === 'screen') await runScreeningPipeline(opportunityId)
      if (action === 'score') { await scoreAndPersist(sql, opportunityId, admin.id); await sql`UPDATE network_opportunities SET status = 'scored', screening_status = CASE WHEN screening_status = 'pending' THEN 'approved' ELSE screening_status END, updated_at = NOW() WHERE id = ${opportunityId}` }
      if (action === 'release') { await scoreAndPersist(sql, opportunityId, admin.id); await sql`UPDATE network_opportunities SET status = 'live', screening_status = 'approved', live_at = COALESCE(live_at, NOW()), updated_at = NOW() WHERE id = ${opportunityId}` }
      if (action === 'match') { const matchResult = await matchContractors(opportunityId, { limit: 10, minScore: 30 }); await logNetworkEvent({ event_type: 'simulation.match_generated', event_category: 'assignment', opportunity_id: opportunityId, admin_user_id: admin.id, data: { simulated: true, total_eligible: matchResult.total_eligible, returned: matchResult.matches.length }, triggered_by: 'admin' }); return NextResponse.json({ success: true, action, match_result: matchResult, opportunities: await loadSimulated(sql) }) }
    ...
    const first = leadKind === 'commercial' ? 'SimCo' : 'Simulated'
    const last = `${opportunityType.replace('_', ' ')} ${leadQuality}`
    const rows = await sql`INSERT INTO network_opportunities (source_type, source_channel, status, homeowner_first_name, homeowner_last_name, homeowner_email, homeowner_phone, address, city, state, zip, monthly_bill, utility_name, roof_age_years, structure_type, stories, estimated_project_value, battery_interest, battery_candidate, intake_notes, raw_payload, intake_metadata) VALUES (${body.source_type ?? 'homeowner_direct'}, 'simulator', 'intake', ${leadQuality === 'bad' ? null : first}, ${last}, ${leadQuality === 'bad' ? 'bad-email' : `sim-${eventId('lead')}@example.test`}, ${leadQuality === 'bad' ? '5555555555' : '5558675309'}, ${body.address ?? '100 Simulator Way'}, ${body.city ?? 'Austin'}, ${body.state ?? 'TX'}, ${body.zip ?? '78701'}, ${body.monthly_bill ?? d.monthly_bill}, ${body.utility_name ?? 'Austin Energy'}, ${d.roof_age_years}, ${d.structure_type}, ${leadKind === 'commercial' ? '2' : '1'}, ${body.estimated_value ?? d.estimated_project_value}, ${d.battery_interest}, ${d.battery_interest}, ${`Simulated ${opportunityType} ${leadKind} lead (${leadQuality}, ${urgency})`}, ${JSON.stringify(simulatedPayload)}, ${JSON.stringify(simulatedPayload)}) RETURNING id`
    ...
    if (body.run_screening) await runScreeningPipeline(id)
    if (body.run_scoring) await scoreAndPersist(sql, id, admin.id)
    if (body.release_to_marketplace) { await scoreAndPersist(sql, id, admin.id); await sql`UPDATE network_opportunities SET status = 'live', screening_status = 'approved', live_at = COALESCE(live_at, NOW()), updated_at = NOW() WHERE id = ${id}` }
    let matchResult = null
    if (body.generate_matches) matchResult = await matchContractors(id, { limit: 10, minScore: 30 })
    ...
    console.error('[POST /api/admin/network/simulator]', error)
