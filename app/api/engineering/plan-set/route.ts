...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      // v47.410 — Optional optimizer SKU cap from request body. Drives NEC
      // 690.8(A)(2) ampacity/OCPD sizing inside computeSystem() when topology
      // === 'optimizer'. Omitted → computeSystem() defaults to 15.0 A.
    ...
    console.log('[ENGINE EXECUTED]', {
        systemModel: systemModel ? 'computed' : 'null',
        stringCount: cs?.strings?.length ?? 0,
        panelCount: cs?.totalPanels ?? 0,
      });
    } catch (csErr: unknown) {
      // If computeSystem fails (e.g. missing equipment data), fall back gracefully
      console.warn('[plan-set] computeSystem failed, using fallback values:', (csErr as Error).message);
      systemModel = null;
    }
    ...
    // ── V4 BOM enrichment (v48.x) ──────────────────────────────────────────
    // Augments base equipment items with real part numbers + NEC refs from
    // the V4 registry when inverterId can be resolved. Non-blocking.
    let enrichedEquipItems = equipItems;
    try {
      enrichedEquipItems = enrichEquipmentItemsFromV4(equipItems, {
        ...
        backfeedBreakerAmps:  resolvedBackfeed,
        acDisconnectAmps:     acDisconnectAmps || resolvedAcOcpd,
        mainPanelAmps:        mainPanelBusAmps || 200,
      });
    } catch (enrichErr) {
      console.warn('[plan-set] V4 enrichment failed (non-critical):', enrichErr);
    }
