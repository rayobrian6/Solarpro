...
async function saveProjectFile(sql: any, params: {
  ...
  try {
    ...
    try {
      ...
      console.warn('[preliminary] saveProjectFile failed:', (e2 as Error).message);
      return false;
    }
  }
}
...
async function saveSvgFile(sql: any, params: {
  ...
  try {
    ...
    try {
      ...
      console.warn('[preliminary] saveSvgFile failed:', (e2 as Error).message);
      return false;
    }
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      console.warn('[preliminary] BOM failed:', (e as Error).message);
    }
    ...
    try {
      ...
      console.warn('[preliminary] SLD failed:', (e as Error).message);
    }
    ...
    if (projectId && user.id) {
      ...
      try {
        ...
        console.warn('[preliminary] production save failed:', (e as Error).message);
      }
      ...
      // FIX v47.293: Read actual project system_type from DB so fence/ground projects
      // get the correct system_type in their seed (not always 'roof').
      let projectSystemType: 'roof' | 'ground' | 'fence' = 'roof';
      try {
        const sqlSt = await getDbReady();
        const stRows = await sqlSt`SELECT system_type FROM projects WHERE id = ${projectId} AND user_id = ${user.id} LIMIT 1`;
        if (stRows.length > 0 && stRows[0].system_type) {
          const raw = String(stRows[0].system_type).toLowerCase().trim();
          if (raw === 'fence' || raw === 'solar_fence') projectSystemType = 'fence';
          else if (raw === 'ground' || raw === 'ground_mount') projectSystemType = 'ground';
          else projectSystemType = 'roof';
          console.log('[preliminary] Project system_type from DB:', projectSystemType, '(raw:', raw, ')');
        }
      } catch (stErr: unknown) {
        console.warn('[preliminary] Could not read project system_type (defaulting to roof):', (stErr as Error).message);
      }
      ...
      try {
        ...
        console.warn('[preliminary] Could not save engineering_seed (run /api/migrate):', (seedErr as Error).message);
      }
      ...
      try {
        ...
        // Physical data improves accuracy even for preliminary reports.
        // If no survey has been done yet, physicalData will be null and
        // the engine falls back to typical residential defaults.
        const _prelPhysData = await getProjectPhysicalData(projectId).catch(() => null);

        // Phase 1+2: Survey pipeline overlay (non-destructive, fully guarded)
        let _prelEnrichedSurvey: EnrichedSiteSurvey | null = null;
        try {
          if (_prelPhysData) {
            const physDataRow = {
              roof_material:          _prelPhysData.roof_material,
              roof_age_years:         _prelPhysData.roof_age_years,
              roof_condition:         _prelPhysData.roof_condition,
              rafter_spacing_in:      _prelPhysData.rafter_spacing_in,
              main_panel_rating_amps: _prelPhysData.panel_rating_amps,
              panel_brand:            _prelPhysData.panel_brand,
              interconnection_point:  _prelPhysData.interconnection_point,
            };
            const _prelRaw = await fromPhysicalData(projectId, physDataRow);
            if (_prelRaw) {
              const _prelNorm = normalizeSurvey(_prelRaw);
              _prelEnrichedSurvey = enrichSurvey(_prelNorm);
              console.log('[SURVEY APPLIED] preliminary projectId:', projectId,
                'photos:', _prelEnrichedSurvey.derived.photoCounts.total,
                'geometry:', _prelEnrichedSurvey.derived.hasGeometryData,
                'electrical:', _prelEnrichedSurvey.derived.hasElectricalData);
            }
          }
        } catch (surveyErr) {
          console.warn('[SURVEY PIPELINE] preliminary skipped (non-fatal):', (surveyErr as Error).message);
        }

        const engReport = generateEngineeringReport(prelimSnapshot as any, engReportId, _prelPhysData, _prelEnrichedSurvey);
        ...
        console.warn('[preliminary] Engineering report auto-gen failed (non-fatal):', (engErr as Error).message);
      }
