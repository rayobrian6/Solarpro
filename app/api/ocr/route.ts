...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      const msg = prepErr instanceof Error ? (prepErr as Error).message : String(prepErr);
...
async function runMultiPassTesseract(
  ...
  if (cliAvailable) {
    ...
    // Pass 1: PSM 3 — auto layout (standard, best for mixed-layout bills)
    ...
    // Pass 2: PSM 6 — single uniform text block (supplement only)
    ...
    // Always prefer PSM 3 — it uses auto layout detection which correctly handles
    // multi-column utility bills, bar charts, and mixed layouts.
    // PSM 6 assumes a single uniform text block which causes digit misreads on
    // structured bill tables (e.g. reads "1906 KWH" as "4906 KWH").
    // Only fall back to PSM 6 if PSM 3 returned almost nothing.
    const best = pass1.text.length >= 50 ? pass1 : (pass2.text.length > pass1.text.length ? pass2 : pass1);
    const bestPsm = best === pass1 ? 3 : 6;
    log.push(`[ocr] Best pass: PSM ${bestPsm} (${best.text.length} chars) — PSM3 preferred for bill accuracy`);
