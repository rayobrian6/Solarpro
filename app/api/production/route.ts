...
export const maxDuration = 30; // Production calc: PVWatts API + 5 DB calls needs headroom
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── Multi-array breakdown ─────────────────────────────────────────────
    const lat = client.lat ?? 33.4484;
    const lng = client.lng ?? -112.074;
    const solarArrays = buildArraysFromLayout(savedLayout, lat, lng);
    const systemConfig = buildSystemConfig(solarArrays);
    const arrayBreakdown = buildArrayBreakdown(systemConfig);
    ...
    const costEstimate = {
      ...
      // v47.182: Per-array production breakdown
      arrayBreakdown:      arrayBreakdown.length > 1 ? arrayBreakdown : undefined,
    };
    ...
    // Log the real error for Vercel function logs (searchable as [PRODUCTION_ERROR])
    ...
    // Only use handleRouteDbError for actual DB-layer errors (DbConfigError or DB cold-start)
    // For non-DB errors, return the real message so the client can display it
    ...
    // Real application error — return the actual message
