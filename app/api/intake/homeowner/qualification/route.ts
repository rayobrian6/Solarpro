...
type QualificationStage = 'request_parse' | 'db_connect' | 'original_event_lookup' | 'qualification_insert'

function qualificationError(stage: QualificationStage, error: unknown, status = 500) {
...
function makeEventId(originalEventId: string): string {
  ...
  return `qual_${originalEventId}_${Date.now()}_${random}`.replace(/[^a-zA-Z0-9_-]/g, '_').slice(0, 180)
}
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const originalEventId = str(body.original_event_id) ?? str(body.event_id) ?? str(body.intake_event_id)
    if (!originalEventId) {
      return qualificationError(stage, new Error('original_event_id is required'), 400)
    }

    const qualification = normalizeQualificationInput(body)
    ...
    if (!original) {
      return qualificationError(stage, new Error('Original homeowner intake event not found'), 404)
    }

    const originalPayload = (original.payload && typeof original.payload === 'object') ? original.payload as Record<string, unknown> : {}
    const context: HomeownerQualificationContext = {
      monthly_bill_amount: num(originalPayload.monthly_bill_amount) ?? num(originalPayload.average_monthly_bill),
      homeowner_status: str(originalPayload.homeowner_status) ?? str(originalPayload.home_ownership),
      ...
      roof_age: str(originalPayload.roof_age) ?? str(originalPayload.roof_age_years),
    }
    const intelligence = deriveQualificationIntelligence(qualification, context)
    const opportunityId = original.opportunity_id ?? null
    const eventId = makeEventId(originalEventId)
    const idempotencyKey = str(body.idempotency_key) ?? `${HOMEOWNER_QUALIFICATION_EVENT_TYPE}:${originalEventId}`
    const payload = {
    ...
    console.error(`[POST /api/intake/homeowner/qualification] stage=${stage} Error:`, err)
