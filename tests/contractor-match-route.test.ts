...
function req(body?: unknown, method = 'POST'): any {
  return new Request('https://solarpro.test/api/admin/network/contractor-match/opp-1', {
    method,
...
function makeSql(opts: { gate?: Record<string, unknown> | null; insertRows?: any[]; oppRows?: any[]; assignments?: any[] } = {}) {
  const calls: string[] = []
  const sql = vi.fn(async (strings: TemplateStringsArray) => {
    const q = strings.join(' ')
    calls.push(q)
    if (q.includes('SELECT no.status, no.screening_status')) return opts.gate === undefined ? [{ status: 'live', screening_status: 'approved', auto_decision: null, override_decision: null }] : (opts.gate ? [opts.gate] : [])
    if (q.includes('INSERT INTO opportunity_assignments')) return opts.insertRows ?? [{ id: 'assignment-1' }]
    if (q.includes('FROM network_opportunities WHERE id')) return opts.oppRows ?? [{ id: 'opp-1', state: 'CA', status: 'live' }]
    if (q.includes('FROM opportunity_assignments oa')) return opts.assignments ?? []
...
describe('/api/admin/network/contractor-match/[id]', () => {
  ...
  it('GET uses canonical opportunity and contractor profile aliases', async () => {
    ...
    const opportunityQuery = sql.calls.find((q: string) => q.includes('FROM network_opportunities WHERE id')) ?? ''
    ...
    const assignmentsQuery = sql.calls.find((q: string) => q.includes('FROM opportunity_assignments oa')) ?? ''
  ...
  it('POST counts actual inserted assignment rows', async () => {
    ...
    const res = await POST(req({ create_assignments: true }), { params: { id: 'opp-1' } })
    expect(res.status).toBe(200)
    expect(await res.json()).toMatchObject({ success: true, assignments_created: 1, total_eligible: 1 })
    expect(mockLogNetworkEvent).toHaveBeenCalledWith(expect.objectContaining({ event_type: 'assignment.offered', data: expect.objectContaining({ source: 'contractor_match', assignments_created: 1 }) }))
  })
