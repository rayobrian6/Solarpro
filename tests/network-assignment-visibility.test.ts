...
vi.mock('@/lib/rateLimiter', () => ({ checkRateLimit: mockCheckRateLimit, getClientIp: mockGetClientIp }))
...
function makeSql() {
  ...
  const sql = vi.fn(async (strings: TemplateStringsArray) => {
    ...
    if (q.includes('FROM opportunities o') && q.includes("o.status = 'open'")) return []
    if (q.includes('SELECT COUNT(*) AS total') && q.includes('FROM opportunities o')) return [{ total: 0 }]

    if (q.includes('SELECT COUNT(*) AS total') && q.includes('FROM opportunity_assignments oa') && q.includes("oa.status IN ('offered','viewed')")) {
      return [{ total: 1 }]
    }

    if (q.includes('FROM opportunity_assignments oa') && q.includes('JOIN network_opportunities no') && q.includes('LIMIT 1')) {
      return [{ assignment_id: ASSIGNMENT_ID, assignment_status: 'offered', opportunity_id: OPP_ID, opportunity_status: 'live', asking_price: 500 }]
    }

    if (q.includes('FROM opportunity_assignments oa') && q.includes("oa.status IN ('offered','viewed')")) {
      return [assignedOpportunity]
    }

    if (q.includes('FROM opportunities') && q.includes('WHERE id =')) return []

    if (q.includes('UPDATE opportunity_assignments') && q.includes("SET status = 'claimed'")) {
      return [{ id: ASSIGNMENT_ID, opportunity_id: OPP_ID, contractor_id: USER_ID, status: 'claimed' }]
    }

    if (q.includes('UPDATE network_opportunities')) return []

    if (q.includes('SELECT') && q.includes('FROM network_opportunities no') && q.includes('WHERE no.id =')) {
      return [claimedOpportunity]
    }

    if (q.includes('FROM opportunity_claims c') && q.includes('JOIN opportunities o')) return []
    if (q.includes('SELECT COUNT(*) AS total') && q.includes('FROM opportunity_claims')) return [{ total: 0 }]

    if (q.includes('FROM opportunity_assignments oa') && q.includes("oa.status IN ('claimed','contacted','appointment','proposal','won')")) {
      return [claimedOpportunity]
    }

    if (q.includes('SELECT COUNT(*) AS total') && q.includes('FROM opportunity_assignments') && q.includes("status IN ('claimed','contacted','appointment','proposal','won')")) {
      return [{ total: 1 }]
    }
...
describe('contractor network assignment visibility', () => {
  beforeEach(() => {
    vi.resetModules()
    mockGetUserFromRequest.mockReset().mockReturnValue({ id: USER_ID, email: 'contractor@test.com' })
  ...
  it('claims an assigned network opportunity through the existing contractor claim endpoint', async () => {
    const { POST } = await import('@/app/api/network/opportunities/[id]/claim/route')

    const res = await POST(req(`https://solarpro.test/api/network/opportunities/${OPP_ID}/claim`, undefined, 'POST'), { params: { id: OPP_ID } })

    expect(res.status).toBe(201)
    const json = await res.json()
    expect(json.success).toBe(true)
    expect(json.opportunity).toMatchObject({ id: OPP_ID, address: '123 Solar Way', claim_status: 'claimed' })
  })
