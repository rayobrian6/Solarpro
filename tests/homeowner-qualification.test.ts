...
function req(body: unknown): any {
  return new Request(
    "https://solarpro.test/api/intake/homeowner/qualification",
    {
      method: "POST",
...
function makeQualificationSql() {
  ...
  const sql = vi.fn(
    async (strings: TemplateStringsArray, ...queryValues: unknown[]) => {
      const q = strings.join(" ");
      queries.push(q);
      values.push(queryValues);
      if (
        q.includes("FROM intake_events") &&
        q.includes("event_type = 'homeowner_intake'")
      ) {
        return [
          {
...
describe("homeowner post-submit qualification intelligence", () => {
  ...
  it("derives qualification status, lead grade, readiness flags, and contractor summary", () => {
    ...
    expect(intelligence.qualification_statuses).toEqual(
    ...
    expect(intelligence.matcher_input).toMatchObject({
  ...
  it("persists qualification as a second intake_events lifecycle event and projects canonical intelligence", async () => {
    ...
    const { POST } =
      await import("@/app/api/intake/homeowner/qualification/route");
    ...
    expect(json).not.toHaveProperty("intelligence");
    ...
    const insertedPayload = sql.values
      .flat()
      .find(
        (value: unknown) =>
          typeof value === "string" && value.includes("estimated_credit_band"),
      ) as string;
    const parsedPayload = JSON.parse(insertedPayload);
    expect(parsedPayload.qualification).toMatchObject({
      purchase_intent: "financing",
      estimated_credit_band: "680_719",
      estimated_income_band: "100k_150k",
      property_type: "single_family",
  ...
  it("accepts qualification answers nested under the public post-submit form payload", async () => {
    const sql = makeQualificationSql();
    mockGetDbReady.mockResolvedValueOnce(sql);
    const { POST } =
      await import("@/app/api/intake/homeowner/qualification/route");
    const res = await POST(
      req({
        original_event_id: "evt_homeowner_test",
        qualification: {
          purchase_intent: "cash",
          estimated_credit_band: "720_plus",
          estimated_income_band: "150k_plus",
          property_type: "commercial",
          electrical_panel_size: "150a",
          sunlight_confidence: "partial_shade",
          prior_quotes: "no",
        },
      }),
    );
    ...
    expect(parsedPayload.qualification_statuses).toEqual(
      expect.arrayContaining(["commercial", "cash_buyer"]),
    );
  });
  ...
  it("feeds qualification intelligence into opportunity scoring dimensions", () => {
    ...
    const scored = scoreOpportunity({
      ...
      qualification_intelligence: intelligence,
    });
    expect(scored.financial.factors).toMatchObject({
      credit_tier_raw: "excellent",
      eligible: true,
      median_income: 175000,
    });
    expect(scored.intent.factors).toMatchObject({
      nlp_intent: intelligence.lead_score,
    });
    expect(scored.opportunity_highlights).toContain(
      "battery_add_on_opportunity",
    );
  });
  ...
  it("adds qualification intelligence into marketplace enrichment projections", () => {
    ...
    expect(payload.marketplace.contractor_summary.value).toContain(
      "A-Grade Opportunity",
    );
    expect(payload.homeowner_sales.financing_probability.factors).toContain(
      "qualification finance readiness",
    );
    expect(payload.roof_install.battery_readiness.factors).toContain(
      "qualification battery readiness",
    );
  });
  ...
  it("surfaces qualification intelligence in Admin Intake Feed API and UI source", () => {
    ...
    expect(routeSource).toContain("q.payload->>'qualification_status'");
    expect(routeSource).toContain("q.payload->>'estimated_credit_band'");
  ...
  it("contractor matcher reads canonical qualification projection without duplicate opportunity systems", () => {
    const matcherSource = fs.readFileSync(
      path.join(process.cwd(), "lib/network/contractorMatcher.ts"),
      "utf8",
    );
