...
describe("admin intake funnels infrastructure", () => {
  ...
  it("keeps migration 071 compatible with the admin SQL splitter", () => {
    ...
    expect(statements.some((statement) => /^does\b/i.test(statement))).toBe(
      false,
    );
  ...
  it("enables Open/Copy/UTM funnel actions from canonicalUrl without requiring campaigns or events", () => {
    expect(adminPageSource).toContain("const canonicalUrl =");
    expect(adminPageSource).toContain(
      "funnel.canonicalUrl || funnel.canonical_url || null",
    );
    expect(adminPageSource).toContain("const generatedUtmUrl = canonicalUrl");
    expect(adminPageSource).toContain("appendClientUtm(canonicalUrl, utm)");
    expect(adminPageSource).toContain(
    ...
    expect(adminPageSource).not.toContain(
      "disabled={!funnel.source_campaign_support",
    );
    expect(adminPageSource).not.toContain("disabled={!funnel.campaign_id");
    expect(adminPageSource).not.toContain(
      "disabled={!funnel.recent_intake_count",
    );
    expect(adminPageSource).not.toContain(
      "disabled={funnel.recent_intake_count",
    );
  ...
  it("surfaces an operational Intake Funnels tab with required copy/open actions", () => {
    ...
    expect(adminPageSource).toContain('cache: "no-store"');
