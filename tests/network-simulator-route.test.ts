...
vi.mock("@/lib/network/screeningPipeline", () => ({
  runScreeningPipeline: mockRunScreening,
}));
...
vi.mock("@/lib/network/opportunityScorer", () => ({
  scoreOpportunity: () => ({
    overall_score: 88.33,
    overall_grade: "A",
    property: { score: 80.25 },
    solar: { score: 90.75 },
    financial: { score: 80 },
    market: { score: 85 },
    intent: { score: 90 },
    risk_flags: [],
    opportunity_highlights: ["high_bill"],
    executive_summary: "Strong simulated opportunity.",
  }),
  scoreToListingPrice: () => ({
    price: 500,
    min: 350,
    max: 700,
    rationale: "test pricing",
  }),
}));

async function importRoute() {
  return import("@/app/api/admin/network/simulator/route");
}
function req(body?: unknown, method = "POST"): any {
  return new Request("https://solarpro.test/api/admin/network/simulator", {
    method,
    headers: {
...
function makeSql() {
  ...
  const sql = vi.fn(async (strings: TemplateStringsArray, ...values: any[]) => {
      ...
      return [
        {
          id: "sim-opp-1",
          location_state: "TX",
          monthly_usage_avg_kwh: 300,
          source_type: "homeowner_direct",
          battery_candidate: true,
        },
      ];
    if (q.includes("AS marketplace_ready"))
      return [
        {
          id: "sim-opp-1",
          status: "live",
    ...
    if (q.includes("SELECT no.id, no.status"))
      return [
        { id: "sim-opp-1", status: "intake", raw_payload: { simulated: true } },
      ];
    return [];
  }) as any;
  sql.queries = queries;
...
describe("/api/admin/network/simulator", () => {
  beforeEach(() => {
    ...
    mockMatchContractors
      .mockReset()
      .mockResolvedValue({
        total_eligible: 1,
        matches: [{ contractor_id: "c1" }],
      });
  ...
  it("creates a simulated opportunity with canonical marker and intake event", async () => {
    ...
    const scoreUpdateIndex = sql.queries.findIndex(
      (q: string) =>
        q.includes("UPDATE network_opportunities SET") &&
        q.includes("opportunity_score"),
    );
    ...
    const intelligenceInsertIndex = sql.queries.findIndex((q: string) =>
      q.includes("INSERT INTO opportunity_intelligence"),
    );
    ...
    const releaseCreate = await POST(
      req({ action: "create", release_to_marketplace: true }),
    );
    expect(releaseCreate.status).toBe(200);
    expect(await releaseCreate.json()).toMatchObject({
      marketplace_ready: { marketplace_ready: true },
    });
  ...
  it("can trigger screening, scoring, release, and matching through existing modules", async () => {
    ...
    const score = await POST(
      req({ action: "score", opportunity_id: "sim-opp-1" }),
    );
    expect(score.status).toBe(200);
    const release = await POST(
      req({ action: "release", opportunity_id: "sim-opp-1" }),
    );
    ...
    const match = await POST(
      req({ action: "match", opportunity_id: "sim-opp-1" }),
    );
    expect(match.status).toBe(200);
    expect(mockMatchContractors).toHaveBeenCalledWith("sim-opp-1", {
      limit: 10,
      minScore: 30,
    });
  });
  ...
  it("persists screening failures to the canonical step10_fail_reasons column and never queries fail_reasons", () => {
    const pipelineSource = fs.readFileSync(
      path.join(process.cwd(), "lib/network/screeningPipeline.ts"),
      "utf8",
    );
    const enrichmentSource = fs.readFileSync(
      path.join(process.cwd(), "lib/network/opportunityEnrichment.ts"),
      "utf8",
    );
    expect(pipelineSource).toContain("step10_fail_reasons   = ${failReasons}");
    expect(pipelineSource).not.toMatch(
      /UPDATE opportunity_screening_queue SET[\s\S]*\bfail_reasons\s*=/,
    );
    expect(enrichmentSource).toContain(
      "step10_fail_reasons, step10_review_flags AS review_flags FROM opportunity_screening_queue",
    );
    expect(enrichmentSource).not.toContain(
      "step10_fail_reasons, review_flags FROM opportunity_screening_queue",
    );
  ...
  it("refreshes simulator UI state after successful create/actions instead of waiting for manual reload", () => {
    ...
    expect(adminPageSource).toContain("if (Array.isArray(data.opportunities))");
  ...
  it("returns stage-aware simulator create failure details without leaking secrets", async () => {
    const sql = vi.fn(async (strings: TemplateStringsArray) => {
      ...
      if (q.includes("INSERT INTO network_opportunities")) {
        const err = new Error(
          'column "homeowner_first_name" of relation "network_opportunities" does not exist',
