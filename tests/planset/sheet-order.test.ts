...
// derive from buildSheetManifest so they can't drift. Reading order groups the
// electrical sheets together (E-1 sits with them, not orphaned after the certs).
describe('planset sheet order', () => {
  ...
  it('groups the electrical sheets: PV-4A → PV-4B → E-1, before structural PV-3', () => {
    expect(at('PV-4A')).toBeGreaterThan(-1);
    expect(at('PV-4A')).toBeLessThan(at('PV-4B'));
    expect(at('PV-4B')).toBeLessThan(at('E-1'));
    expect(at('E-1')).toBeLessThan(at('PV-3'));      // E-1 no longer after the certs
    expect(at('PV-3')).toBeLessThan(at('PV-4C'));
  });
  ...
  it('buildSheetManifest is the single ordered source (E-1 with electrical)', () => {
    const m = buildSheetManifest({ pv1Title: 'A', pv3Title: 'B' }).map(s => s.id);
    expect(m.indexOf('PV-4B')).toBeLessThan(m.indexOf('E-1'));
    expect(m.indexOf('E-1')).toBeLessThan(m.indexOf('PV-3'));
