...
export default function BillUploadModal({ onClose, onComplete }: BillUploadModalProps) {
  ...
  // Client-side system size fallback — used when /api/system-size doesn't return sizing
  // (e.g. geocoding failure, timeout, cold-start). Production factor by state from utility-rules.
  const computeSystemKwFallback = useCallback((bd: any): number => {
    const annualKwh =
      bd?.estimatedAnnualKwh ||
      bd?.annualKwh ||
      (bd?.monthlyUsageHistory?.length >= 10
        ? bd.monthlyUsageHistory.reduce((s: number, v: number) => s + v, 0)
        : bd?.monthlyUsageHistory?.length >= 3
          ? Math.round((bd.monthlyUsageHistory.reduce((s: number, v: number) => s + v, 0) / bd.monthlyUsageHistory.length) * 12)
          : 0) ||
      (bd?.monthlyKwh ? bd.monthlyKwh * 12 : 0);
    if (!annualKwh || annualKwh <= 0) return 0;
    // Default production factor 1250 kWh/kW/yr (national average)
    const kw = Math.round((annualKwh * 1.0 / 1250) * 10) / 10;
    console.log(`[BillUploadModal] Fallback sizing: annualKwh=${annualKwh} → ${kw} kW`);
    return kw;
  }, []);
  ...
  const runSystemSizing = useCallback(async (billData: any, uploadResult: any) => {
    ...
    try {
      const sizingRes = await fetch('/api/system-size', {
        ...
        body: JSON.stringify({
          // Prefer sum of full monthly history (12 months) over estimatedAnnualKwh
          // which may be extrapolated from a smaller sample or stale AI-fallback value
          annual_kwh: (() => {
            const hist = (billData.monthlyUsageHistory as number[] | undefined)?.filter((v: number) => v > 0) || [];
            if (hist.length >= 10) return hist.reduce((s: number, v: number) => s + v, 0);
            return billData.estimatedAnnualKwh || billData.annualKwh || null;
          })(),
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      // v47.213: Hard 57s client-side timeout — prevents infinite spinner if Vercel cuts the connection
      const uploadAbort = new AbortController();
      const uploadTimeout = setTimeout(() => uploadAbort.abort(), 57000);
      let res: Response;
      try {
        res = await fetch('/api/bill-upload', { method: 'POST', body: formData, signal: uploadAbort.signal });
      } catch (fetchErr: unknown) {
        clearTimeout(uploadTimeout);
        const msg = fetchErr instanceof Error && (fetchErr as Error).name === 'AbortError'
          ? 'Bill processing timed out. Please try a smaller file or enter your usage manually.'
          : (fetchErr instanceof Error ? (fetchErr as Error).message : 'Network error. Please try again.');
        cancelSim();
        throw new Error(msg);
      }
      clearTimeout(uploadTimeout);
      ...
      cancelSim();
      setError(err instanceof Error ? (err as Error).message : 'Upload failed. Check your connection.');
      setProcessingStage('uploading');
    } finally {
      setUploading(false);
    }
  ...
  const handleManualEntry = useCallback(async () => {
    ...
    try {
      ...
      // v47.213: Hard 20s client-side timeout for manual text submission
      const manualAbort = new AbortController();
      const manualTimeout = setTimeout(() => manualAbort.abort(), 20000);
      let res: Response;
      try {
        res = await fetch('/api/bill-upload', { method: 'POST', body: formData, signal: manualAbort.signal });
      } catch (fetchErr: unknown) {
        clearTimeout(manualTimeout);
        throw new Error(fetchErr instanceof Error && (fetchErr as Error).name === 'AbortError'
      ...
      setError(err instanceof Error ? (err as Error).message : 'Request failed');
    } finally {
      setUploading(false);
    }
  ...
  const handleCreateAll = useCallback(async () => {
    ...
    try {
      // ── Step 1: Provision client + project + hub via single atomic endpoint ──
      log('Creating client & project...');

      // Compute authoritative annual kWh: prefer sum of full monthly history
      const history = result.billData.monthlyUsageHistory?.filter((v: number) => v > 0) || [];
      const historyAnnual = history.length >= 10
        ? history.reduce((s: number, v: number) => s + v, 0)
        : history.length >= 3
          ? Math.round(history.reduce((s: number, v: number) => s + v, 0) / history.length * 12)
          : 0;
      const billDataForProvision = {
        ...result.billData,
        // Override estimatedAnnualKwh with authoritative value from monthly history when available
        estimatedAnnualKwh: historyAnnual > 0
          ? historyAnnual
          : (result.billData.estimatedAnnualKwh || result.billData.annualKwh || 0),
        annualKwh: historyAnnual > 0
          ? historyAnnual
          : (result.billData.annualKwh || result.billData.estimatedAnnualKwh || 0),
      };

      const provRes = await fetch('/api/bill-upload/provision', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          billData:    billDataForProvision,
          locationData: result.locationData,
          utilityData:  result.utilityData,
          systemKw:     systemKw || undefined,
        }),
      });
      const provData = await provRes.json();
      if (!provData.success) {
        throw new Error('Failed to create client & project: ' + (provData.error || 'Unknown error'));
      }
      const clientId   = provData.clientId   as string;
      const projectId  = provData.projectId  as string;
      const clientName = provData.clientName as string || result.billData.customerName || 'New Customer';
      const projectName = provData.projectName as string || `${clientName} — Solar ${new Date().getFullYear()}`;
      log(`✓ Client created: ${clientName}`);
      log(`✓ Project created: ${projectName}`);

      // ── Step 2: Auto-generate engineering workspace ───────────────────────────
      log('Building engineering workspace...');
      try {
        const annualKwhForPacket = (billDataForProvision.estimatedAnnualKwh ?? 0) > 0
          ? billDataForProvision.estimatedAnnualKwh
          : (result.billData.monthlyKwh ?? 0) > 0
            ? result.billData.monthlyKwh! * 12
            : 0;

        const safeAddress = result.billData.serviceAddress?.trim() || (
          result.locationData
        ...
        if (annualKwhForPacket === 0) {
          ...
          if (prelimData.success) {
            ...
            if (fileList.length > 0) {
              log(`✓ Engineering workspace created (${fileList.length} files)`);
            }
            if (hasEngReport) {
        ...
        const msg = pkgErr instanceof Error ? (pkgErr as Error).message : String(pkgErr);
        log(`⚠ Engineering workspace skipped: ${msg}`);
      }

      // ── Step 3: Save original bill PDF/image as project file ─────────────────
      if (lastFile) {
        try {
      ...
      const msg = err instanceof Error ? (err as Error).message : 'Creation failed';
      log(`✗ Error: ${msg}`);
      setError(msg);
    }
  }, [result, systemKw, lastFile, onComplete]);
              ...
              {result.billData.billInsights ? (
                <div className="bg-slate-800/60 border border-slate-600/50 rounded-lg p-3 text-xs">
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className="text-sky-300 font-semibold uppercase tracking-wide text-[10px]">Bill Analysis</span>
                    <span className={`ml-auto px-1.5 py-0.5 rounded text-[10px] font-medium ${
                      result.billData.billInsights.confidence === 'high'   ? 'bg-emerald-500/20 text-emerald-300' :
                      result.billData.billInsights.confidence === 'medium' ? 'bg-amber-500/20 text-amber-300' :
                      'bg-slate-500/20 text-slate-400'
                    }`}>
                      {result.billData.billInsights.confidence} confidence
                    </span>
                  </div>
                  <div className="space-y-1.5">
                    {/* Combined utility row */}
                    <div className="flex items-center justify-between">
                      <span className="text-slate-400">Combined Utility Bill</span>
                      <span className={`font-medium ${result.billData.billInsights.combinedUtilityDetected ? 'text-amber-400' : 'text-emerald-400'}`}>
                        {result.billData.billInsights.combinedUtilityDetected ? 'YES' : 'NO'}
                      </span>
                    </div>
                    {/* Detected services */}
                    ...
                    {result.billData.billInsights.suggestedRate !== null ? (
                      ...
                      result.billData.billInsights.combinedUtilityDetected && (
                        <div className="flex items-center justify-between">
                          <span className="text-slate-400">Suggested Electric Rate</span>
                          <span className="text-slate-500 italic">not available — mixed bill</span>
                        </div>
                      )
                    )}
          ...
          {step === 'done' && created ? (
            <div className="space-y-4 py-4 text-center">
              ...
              <div className="flex gap-3">
                ...
                <a href={`/design?projectId=${created.projectId}`} className="btn-primary flex-1 py-2 text-sm text-center flex items-center justify-center gap-1">
                  Design Studio <ArrowRight size={14} />
                </a>
              </div>
              <div className="flex gap-2 mt-1">
                <a
                  href={`/engineering?projectId=${created.projectId}`}
                  className="flex-1 text-center text-xs text-blue-400 hover:text-blue-300 underline underline-offset-2 block"
                >
                  Engineering →
                </a>
                <a
                  href={`/projects/${created.projectId}`}
                  className="flex-1 text-center text-xs text-slate-400 hover:text-slate-300 underline underline-offset-2 block"
                >
                  Project Page →
                </a>
              </div>
