...
export default function BillUploadFlow({ onComplete, onClose, className = '' }: BillUploadFlowProps) {
  ...
  const computeSystemKwFallback = useCallback((bd: any): number => {
    ...
    console.log(`[BillUploadFlow] Fallback sizing: annualKwh=${annualKwh} → ${kw} kW`);
    return kw;
  }, []);
      ...
      if (sizingData.success) {
        ...
        try {
          const provRes = await fetch('/api/bill-upload/provision', {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              billData: merged.billData,
              locationData: merged.locationData,
              utilityData: merged.utilityData,
              systemKw: resolvedKw,
            }),
          });
          const provData = await provRes.json();
          if (provData.success && provData.projectId) {
            setProvisionedProjectId(provData.projectId);
            router.push(`/design?projectId=${provData.projectId}`);
            return;
          } else {
            console.warn('[BillUploadFlow] provision failed:', provData.error);
          }
        } catch (provErr: unknown) {
          console.warn('[BillUploadFlow] provision threw:', provErr instanceof Error ? (provErr as Error).message : provErr);
        } finally {
  ...
  }, [router, computeSystemKwFallback]);
  ...
  return (
    <div className={`relative bg-slate-900 rounded-2xl border border-slate-700/50 overflow-hidden ${className}`}>
      {/* Header */}
      ...
      {provisioning ? (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-slate-900/95 rounded-2xl gap-4">
          ...
          <div className="text-center">
            <p className="text-white font-semibold text-sm">Setting up your project…</p>
