 ...
 *
 *   ANTHROPIC_API_KEY     — Claude 3.5 Sonnet bill extraction (primary AI engine)
 *                           Without this, falls back to GPT-4o-mini for text extraction
 *                           and loses multimodal image parsing for uploaded bill images
 *   OPENAI_API_KEY        — GPT-4o-mini bill extraction fallback (used when Claude unavailable)
 *                           Without this AND ANTHROPIC_API_KEY, falls back to Tesseract OCR only
...
export function logEnvStatus(routeLabel: string): void {
  ...
  const keyVars = ['DATABASE_URL', 'JWT_SECRET', 'ANTHROPIC_API_KEY', 'OPENAI_API_KEY', 'GOOGLE_MAPS_API_KEY', 'RESEND_API_KEY', 'NEXT_PUBLIC_BASE_URL'];
...
function getVarWarning(varName: string): string {
  const warnings: Record<string, string> = {
    ANTHROPIC_API_KEY:      'Claude 3.5 Sonnet bill extraction disabled — falling back to GPT-4o-mini',
    OPENAI_API_KEY:         'GPT-4o-mini bill extraction fallback disabled (Tesseract OCR only if Claude also absent)',
