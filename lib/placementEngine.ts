...
export function calcEffectiveSetback(config: FireSetbackConfig): number {
  // v47.111: Returns SIDE setback only (used as uniform polygon shrink for the visual overlay).
  // The CAD solver now uses per-edge setbacks directly (ridge/eave/side separate).
  // Pathway is a fire access LANE filtered post-grid -- NOT included here.
  return config.edgeSetbackM;
}

/**
 * Returns per-edge setbacks for the CAD solver.
 * eave=0 by default (IRC does not require eave clearance).
 * ridge=18" (IRC R324.4.1).
 * side=18-36" (IRC R324.4.1 / AHJ).
 */
export function getPerEdgeSetbacks(config: FireSetbackConfig): {
  eaveM: number;
  ridgeM: number;
  sideM: number;
} {
  return {
    eaveM:  config.eaveSetbackM  ?? 0,
    ridgeM: config.ridgeSetbackM ?? 0.457,
    sideM:  config.edgeSetbackM  ?? 0.457,
  };
}
