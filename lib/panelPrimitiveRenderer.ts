...
export function buildPanelPrimitive(
  ...
  for (const panel of panels) {
    try {
      ...
      // Fix 1: Lock heading to roof azimuth — no independent panel rotation
      const heading  = panel.azimuth * Math.PI / 180;
  ...
  try {
    ...
    console.error('[PrimitiveRenderer] buildPanelPrimitive failed:', (e as Error).message);
    return null;
  }
}
...
export function updatePrimitiveColors(
  ...
  try {
    ...
    console.warn('[PrimitiveRenderer] updatePrimitiveColors error:', (e as Error).message);
  }
}
