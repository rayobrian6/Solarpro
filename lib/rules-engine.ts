...
export function runRulesEngine(input: RulesEngineInput): RulesEngineResult {
  ...
  // v57.5 — Detect optimizer topology. NEC 690.7 Voc string voltage check does NOT
  // apply to optimizer systems (SolarEdge P-series, Tigo TS4-A-O, etc.). Each DC-DC
  // optimizer independently regulates its module contribution; the inverter holds the
  // DC bus in its MPPT window (~380-400V) regardless of panel count. Only the
  // maxPanelsPerString brand limit applies. Same logic as electrical-calc.ts line ~413.
  const _firstElecInv = input.electrical.inverters[0] as any;
  const _isOptimizerTopo = _firstElecInv?.type === 'optimizer'
    || String((input.electrical as any)?.topologyType ?? '').toUpperCase() === 'STRING_OPTIMIZER'
    || String((input.electrical as any)?.topologyType ?? '').toUpperCase() === 'STRING_WITH_OPTIMIZER';

  electricalResult.inverters?.forEach((inv: any) => {
    inv.strings?.forEach((str: any) => {
      dependencyChain.push('NEC_690_7_VOLTAGE');
      if (_isOptimizerTopo) {
        // Optimizer topology: NEC 690.7 Voc check is inapplicable — pass with info
        rules.push({ ruleId: 'NEC_690_7_VOLTAGE', category: 'electrical', severity: 'pass',
          title: 'NEC 690.7 Voltage — Optimizer (N/A)',
          message: 'Optimizer topology: DC-DC optimizers regulate string voltage independently. NEC 690.7 Voc limit does not apply.',
          necReference: 'NEC 690.7(A)' } as any);
      } else {
