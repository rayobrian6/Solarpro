...
import { isLegacyStructuralStatus, LEGACY_PATH_BANNER } from './legacy-path-guard';
...
export function buildStructuralSheet(inp: StructuralSheetInput): string {
  ...
  // W3.1 §3 — PARALLEL-PATH CONTAINMENT (fail closed). When this sheet is produced
  // by the legacy plan-set path (no canonical PermitDesignSnapshot consumed), the
  // structural status is LEGACY_NOT_FOR_PERMIT and NO "STRUCTURAL PASS" is
  // declared. The load analysis is still rendered for information.
  const isLegacy = isLegacyStructuralStatus(inp.structuralStatus);

  const legacyBannerHtml = isLegacy ? `
    <div style="margin-top:4px; margin-bottom:2px; padding:6px 10px; border:2px solid #b00020; border-radius:4px; background:#fff0f0; text-align:center;">
      <div style="font-size:9pt; font-weight:bold; color:#b00020; letter-spacing:0.3px;">${escHtml(LEGACY_PATH_BANNER)}</div>
      <div style="font-size:5.8pt; color:#7a1020; margin-top:2px;">Generated by the legacy plan-set path, which does not consume the canonical PermitDesignSnapshot. Structural status is NOT certified. Informational only.</div>
    </div>` : '';

  const statusBoxHtml = isLegacy ? `
        <div style="margin-top:8px; padding:8px; border:2px solid #b00020; border-radius:4px; background:#fff0f0; text-align:center;">
          <div style="font-size:11pt; font-weight:bold; color:#b00020;">LEGACY PATH — NOT FOR PERMIT</div>
          <div style="font-size:6.5pt; color:#7a1020; margin-top:3px;">
            Pending canonical migration. No structural PASS is declared on this path (fail closed).
            Load analysis below is informational only.
          </div>
        </div>` : `
        <div style="margin-top:8px; padding:8px; border:2px solid ${'#1a7a1a'}; border-radius:4px; background:${'#f0fff0'}; text-align:center;">
          <div style="font-size:13pt; font-weight:bold; color:${'#1a7a1a'};">✓ STRUCTURAL PASS</div>
  ...
  const content = `
    ...
    <div class="three-col" style="gap:10px; margin-top:4px;">
      ...
      <div>
        ...
        <!-- Status Box — PASS only under a validated canonical snapshot; else LEGACY -->
        ${statusBoxHtml}
      </div>
