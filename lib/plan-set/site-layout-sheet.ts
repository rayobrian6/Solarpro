...
function buildRoofPlanSvg(inp: SiteLayoutSheetInput): string {
  ...
  if (roofPanels.length > 0) {
    ...
    const spanX = Math.max(maxXm - minXm, 0.1);
    const spanY = Math.max(maxYm - minYm, 0.1);
    const scaleX = arrayW / (spanX + scaledPanelW / scale);
    const scaleY = arrayH / (spanY + scaledPanelH / scale);
    for (const p of roofPanels) {
      const px = arrayX + ((p.xMeters ?? 0) - minXm) * scaleX;
      const py = arrayY + arrayH - ((p.yMeters ?? 0) - minYm) * scaleY - scaledPanelH; // flip Y
      panelRects += `<rect x="${px.toFixed(1)}" y="${py.toFixed(1)}" width="${scaledPanelW.toFixed(1)}" height="${scaledPanelH.toFixed(1)}" fill="#ffd700" fill-opacity="0.85" stroke="#e6a817" stroke-width="0.6"/>`;
      count++;
    }
  } else {
    // Schematic fallback: evenly-spaced grid
