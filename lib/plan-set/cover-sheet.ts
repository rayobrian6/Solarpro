...
export function buildCoverSheet(inp: CoverSheetInput): string {
  const content = `
    ...
    ${inp.satelliteImageBase64 ? `
    <!-- Aerial / Vicinity Map -->
    <div style="margin-top:8px;">
      <div class="section-header" style="margin-bottom:4px;">Aerial / Vicinity Map</div>
      <div style="position:relative; border:0.5px solid #aab; border-radius:2px; overflow:hidden; height:140px; background:#eef;">
        <img
          src="${inp.satelliteImageBase64}"
          alt="Aerial view"
          style="width:100%; height:100%; object-fit:cover; display:block;"
        />
        <div style="position:absolute; bottom:4px; left:6px; background:rgba(0,0,0,0.65); color:#fff; font-size:6pt; padding:2px 6px; border-radius:2px;">
          Site aerial view &mdash; ${escHtml(inp.siteAddress)}, ${escHtml(inp.city)}, ${escHtml(inp.state)}
        </div>
      </div>
    </div>` : `
    <!-- Aerial placeholder -->
    <div style="margin-top:8px;">
      <div class="section-header" style="margin-bottom:4px;">Aerial / Vicinity Map</div>
      <div style="border:0.5px solid #aab; border-radius:2px; height:140px; background:#f5f5fa; display:flex; align-items:center; justify-content:center; flex-direction:column; gap:4px;">
        <div style="font-size:8pt; color:#888;">Satellite Image Unavailable</div>
        <div style="font-size:6.5pt; color:#aaa;">${escHtml(inp.siteAddress)}, ${escHtml(inp.city)}, ${escHtml(inp.state)}</div>
        <div style="font-size:6pt; color:#bbb;">Run engineering pipeline to load aerial imagery</div>
      </div>
    </div>`}
