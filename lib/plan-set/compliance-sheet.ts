...
export function buildComplianceItems(inp: ComplianceSheetInput): ComplianceItem[] {
  ...
  // ── Structural ────────────────────────────────────────────────────────────
  // Never show FAIL — FAIL → REVIEW internally. W3.1 §3: the legacy plan-set path
  // (no canonical PermitDesignSnapshot) can NEVER show PASS here — it is REVIEW.
  const isLegacyPath = isLegacyStructuralStatus(inp.structuralStatus);
  ...
  items.push({
    ...
    note: isLegacyPath
      ? 'LEGACY PATH — structural compliance NOT certified. Generated without the canonical PermitDesignSnapshot; pending canonical migration.'
      : structStatus === 'PASS'
...
export function buildComplianceSheet(inp: ComplianceSheetInput): string {
  ...
  // W3.1 §3 — fail closed: the legacy plan-set path can never present a certified
  // compliance status. A prominent NOT-FOR-PERMIT banner is stamped on the sheet.
  const isLegacy = isLegacyStructuralStatus(inp.structuralStatus);
  const legacyBannerHtml = isLegacy ? `
    <div style="margin-top:4px; margin-bottom:6px; padding:6px 10px; border:2px solid #b00020; border-radius:4px; background:#fff0f0; text-align:center;">
      <div style="font-size:9pt; font-weight:bold; color:#b00020; letter-spacing:0.3px;">${escHtml(LEGACY_PATH_BANNER)}</div>
      <div style="font-size:5.8pt; color:#7a1020; margin-top:2px;">Generated by the legacy plan-set path (no canonical PermitDesignSnapshot). Overall compliance is NOT certified.</div>
    </div>` : '';
    ...
    <div style="display:flex; gap:10px; margin-top:8px; border-top:1px solid #ccd; padding-top:6px;">
      ...
      <div style="flex:2; padding:8px; border:1px solid #ccd; border-radius:3px; background:#f7f8fc;">
        ...
        <div style="font-size:6pt; color:#555; line-height:1.6;">
          ${isLegacy ? `
          <strong style="color:#b00020;">LEGACY PATH — NOT FOR PERMIT.</strong> This checklist was generated by the legacy plan-set path,
          which does not consume or validate the canonical PermitDesignSnapshot. It makes NO compliance certification and is
          pending canonical migration. Do not submit for permit on the basis of this sheet.
          ` : `
