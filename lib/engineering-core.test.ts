...
function makeMicroInput(moduleCount: number): MicroSystemInput {
  return {
...
describe('Test 2: String topology — DC string array + Voc/Isc correction', () => {
  ...
  test('each string has corrected Voc (NEC 690.7 temperature correction)', () => {
    ...
    for (const str of result.strings) {
      // vocCorrected must be > 0 (cold temp correction increases Voc)
      expect(str.vocCorrected).toBeGreaterThan(0);
    }
  });
  ...
  test('each string has corrected Isc (NEC 690.8 temperature correction)', () => {
    const result = calcStringSystem(makeStringInput());
    for (const str of result.strings) {
      expect(str.iscCorrected).toBeGreaterThan(0);
    }
  });
...
describe('Test 4: Roof Tech Mini mount → discrete load model structural math', () => {
  ...
  test('rooftech-mini safetyFactor = effectiveCapacity / upliftPerAttachment', () => {
    ...
    const expectedSF = result.attachment.lagBoltCapacity / result.attachment.upliftPerAttachment;
    expect(result.attachment.safetyFactor).toBeCloseTo(expectedSF, 2);
  });

  test('rooftech-mini FAIL only if safetyFactor < 1.0 (ICC-ES discrete model threshold)', () => {
    const input = makeStructuralInput('rooftech-mini');
    const result = runStructuralCalc(input);

    // Implementation uses SF >= 1.0 for discrete model (ICC-ES allowable already factored)
    if (result.attachment.safetyFactor >= 1.0) {
...
describe('Test 5: IronRidge XR100 → distributed load model unchanged', () => {
  ...
  test('IronRidge structural calc uses NDS lag bolt withdrawal formula', () => {
    ...
    // NDS formula: 246 lbs/inch (5/16" lag, DFir, NDS Table 12.2A) × 2.5" embedment × 1.6 CD = 984 lbs
    const expectedLagBoltCapacity = 246 * 2.5 * 1.6;
  ...
  test('IronRidge result is independent of Roof Tech Mini changes', () => {
    ...
    // IronRidge should use NDS formula (246 lbs/in × 4" × 1.6 CD = 984 lbs)
    expect(ironRidgeResult.attachment.lagBoltCapacity).toBeCloseTo(984, 1);
  ...
  test('IronRidge FAIL only if safetyFactor < 1.5 (NDS ASD minimum threshold)', () => {
    const input = makeStructuralInput('ironridge-xr100');
    const result = runStructuralCalc(input);

    // Implementation uses SF >= 1.5 for distributed model (NDS ASD allowable values)
    if (result.attachment.safetyFactor >= 1.5) {
      const sfError = result.errors.find(e => e.code === 'E-ATTACHMENT-SF');
      expect(sfError).toBeUndefined();
    } else {
      const sfError = result.errors.find(e => e.code === 'E-ATTACHMENT-SF');
