...
const PIPELINE_B_CLASS_MAP: Record<string, UnifiedGeometryClass> = {
  ...
  // Raw Pipeline B plane candidates are review evidence only. Only validated
  // consensus_plane_candidate artifacts may become drawable roof geometry.
  plane_candidate:              'unknown',
  roof_plane_candidate:         'unknown',
  wall_plane_candidate:         'unknown',
...
function adaptPlaneCandidate(artifact: PlaneCandidate, surveyId: string): UnifiedGeometryArtifact {
  ...
  return makeEmptyArtifact({
    ...
    provenance,
    confidence: artifact.confidence,
    label: isSynth ? `⚠️ Synthetic: Review-only plane candidate (inliers: ${artifact.inlierCount}/${artifact.totalPoints})` : `Review-only plane candidate (inliers: ${artifact.inlierCount}/${artifact.totalPoints})`,
    limitations: [...artifact.limitations],
    bbox: artifact.region ? regionToBBox(artifact.region) : null,
    planeType: 'roof',
...
function adaptRoofPlaneCandidate(artifact: RoofPlaneCandidate, surveyId: string): UnifiedGeometryArtifact {
  ...
  return makeEmptyArtifact({
    ...
    provenance,
    confidence: artifact.confidence,
    label: isSynth ? `⚠️ Synthetic: Review-only roof plane candidate (inliers: ${artifact.inlierCount}/${artifact.totalPoints})` : `Review-only roof plane candidate (inliers: ${artifact.inlierCount}/${artifact.totalPoints})`,
    limitations: [...artifact.limitations],
    bbox,
    polygon,
    planeType: 'roof',
...
function adaptWallPlaneCandidate(artifact: WallPlaneCandidate, surveyId: string): UnifiedGeometryArtifact {
  ...
  return makeEmptyArtifact({
    ...
    provenance,
    confidence: artifact.confidence,
    label: isSynth ? `⚠️ Synthetic: Review-only wall plane candidate (inliers: ${artifact.inlierCount}/${artifact.totalPoints})` : `Review-only wall plane candidate (inliers: ${artifact.inlierCount}/${artifact.totalPoints})`,
    limitations: [...artifact.limitations],
    bbox,
    polygon,
    planeType: 'wall',
