...
export interface AutoDesignInput {
  ...
  panelWidthM?: number;      // default 1.133m (PS-MNB108)
  panelHeightM?: number;     // default 1.721m (PS-MNB108)
  // Layout settings
...
export function getInterRowSpacing(panelHeightM: number, tiltDeg: number, lat: number): number {
  ...
  // v47.161: Solar elevation at winter solstice noon (correct for both hemispheres)
  // Northern hemisphere: lowest sun at winter solstice = 90 - lat - 23.45
  // Southern hemisphere: lowest sun at summer solstice (June) = 90 + lat - 23.45
  const absLat = Math.abs(lat);
  const solarElevation = 90 - absLat - 23.45;
...
function generatePanelLayout(
  ...
  // v47.161: Use max(rowSpacingM, interRowSpacing) so caller setting is honoured
  const interRowSpacing = Math.max(rowSpacingM, getInterRowSpacing(panelHeightM, tilt, lat));
...
export async function generateAutoDesign(input: AutoDesignInput): Promise<AutoDesignResult> {
  const {
    ...
    roofAreaSqFt,
    panelWatts = 430,
    panelWidthM = 1.133,   // v47.161: PS-MNB108 datasheet: 1133mm
    panelHeightM = 1.721,   // v47.161: PS-MNB108 datasheet: 1721mm
    setbackM = 0.3,
    rowSpacingM = 0.5,
    panelSpacingM = 0.02,
    optimizeTilt = true,
