 ...
 * v47.217: Added fence_mount/ground_mount/roof_mount aliases and human-readable
 * label round-trip safety. Fallback logs warning instead of silently defaulting.
 */
export function normalizeSystemType(systemType: string): SystemTypeString {
  const map: Record<string, SystemTypeString> = {
    ...
    fence:           'fence',
    carport:         'carport',
    // Long-form canonical (design.installType)
    roof_mount:      'roof',
    ground_mount:    'ground',
    fence_mount:     'fence',
  ...
  const normalized = map[systemType];
  if (!normalized) {
    if (typeof systemType === 'string' && systemType.length > 0) {
      console.warn(`[normalizeSystemType] Unknown systemType: "${systemType}" — falling back to 'roof'. Check data source.`);
    }
    return 'roof';
  }
  return normalized;
