...
export const ROADMAP_ITEMS: RoadmapItem[] = [
  ...
  // ─── PLANNED — ENGINEERING CORE ──────────────────────────────────────
  {
    ...
    status: 'done',
    effort: 'l',
    files: ['lib/jurisdiction.ts', 'lib/system/sizingEngine.ts', 'app/api/engineering/calculate/route.ts'],
    notes: 'Shipped at STATE granularity: lib/jurisdiction.ts holds a real per-state ASHRAE design-temp table (coldest city per state, FGIA/ASHRAE HoF) → designTempMin feeds the NEC 690.7 cold-Voc clamp in sizingEngine (replaces the flat -10°C). Future refinement: zip/lat-long granularity instead of per-state.',
    createdAt: '2026-05-10',
  ...
  },
  {
    ...
    shippedIn: 'v47.432',
    notes: 'Done: the assessment was delivered in v47.431 (docs/STAGE7_CONSOLIDATION_ASSESSMENT.md) and the recommended racking drift-fence shipped in v47.432 (lib/system/rackingDatabaseDriftGuard.test.ts). The actual DB merge (execution, Stage 8.3 / the item below) remains deferred — verdict was "keep both behind a drift-guard".',
    createdAt: '2026-05-11',
    updatedAt: '2026-06-22',
  },
  {
  ...
  // ─── PLANNED — API & INTEGRATIONS ────────────────────────────────────
  {
    ...
    status: 'done',
    effort: 'm',
    files: ['lib/pvwatts.ts', 'lib/autoSizing.ts', 'lib/engineering/reportGenerator.ts'],
    notes: 'lib/pvwatts.ts calls the NREL PVWatts v8 API (PVWATTS_API_KEY) with a local fallback; feeds production estimate + shade derate. Note: pvwatts savings/payback outputs are intentionally NOT used in the proposal view (see header comment).',
    createdAt: '2026-05-10',
    updatedAt: '2026-06-22',
  },
  {
  ...
  // ═══ 2026-06 — RECENT WORK (catch-up, all on dev branch) ═══════════════
  {
    id: 'aerial-roof-geometry-nearmap-eagleview',
    title: 'Real aerial roof geometry — Nearmap + EagleView',
    summary: 'The long-sought fix for the dead ground-photo CV path: pull real roof planes (polygon + pitch + azimuth) from licensed aerial AI, feeding the existing UnifiedGeometryArtifact → CanonicalBuildingModel → 16-sheet planset rail. Nearmap AI Feature API (roof-face vectors) + DSM (3D pitch/azimuth); EagleView Measurement Orders as the permit-grade backup. This is the Aurora-parity unlock.',
    track: 'api-integrations',
    priority: 'p0',
    status: 'in-progress',
    effort: 'xl',
    files: ['lib/aerial/nearmap.ts', 'lib/aerial/nearmap.test.ts'],
    notes: 'Nearmap eval key validated 2026-06-22 — API works, eval has AI features. Built lib/aerial/nearmap.ts (server-only, env NEARMAP_API_KEY, fail-safe, coverage-gated): Coverage API + AI Feature API → roof planes (real geo polygon + area + pitch + material + type). Verified live: Edwardsville roofs returned real polygons + pitch (22.6deg / 1.6deg). COVERAGE: covers populated areas (Granite City/Edwardsville/Belleville/Highland + metro) but NOT deep rural (Pocahontas = 0 surveys) — USGS LIDAR fallback planned. PITCH = yes; AZIMUTH = follow-up (not in 2D AI; derive from facet/DSM). NEXT: wire roof planes into the UnifiedGeometryArtifact rail; EagleView still a backup. Each AI call costs credits — gated + cache.',
    createdAt: '2026-06-22',
    updatedAt: '2026-06-22',
  },
  {
    id: 'ahj-national-overhaul',
    title: 'National AHJ database — real data, not fabricated defaults',
    summary: 'Expanded the AHJ database 177 → 4,029 jurisdictions (curated-authoritative + national coverage, merged at load). Made fire setbacks code-driven from the adopted-code table (no fabricated per-AHJ values). Overlaid real NREL SolarTRACE data (online / instant / SolarAPP+ permitting, median permit cost + days) for 1,097 matched AHJs. Wired a fail-safe live SunSpec / Orange Button AHJ Registry lookup for real per-address codes + contacts.',
    track: 'compliance',
    priority: 'p1',
    status: 'in-progress',
    effort: 'l',
    commits: ['316cf77b', 'f63dfa1d', '42fbfe48', 'd3ce3390'],
    files: ['lib/jurisdictions/ahj-national.ts', 'lib/jurisdictions/solartrace.ts', 'lib/jurisdictions/ahjRegistry.ts'],
    notes: 'Live registry blocked on a free SunSpec API token (env AHJ_REGISTRY_TOKEN). Everything else shipped to dev. Guiding principle: no "better than nothing" fabricated data presented as engineered fact.',
    createdAt: '2026-06-21',
    updatedAt: '2026-06-22',
  },
  {
    id: 'fence-solfence-engineering',
    title: 'Fence / SolFence — make fence a first-class mount type',
    summary: 'Engineering is for fences + ground mounts, not just roofs. Built analyzeFence (freestanding-wall wind per ASCE 7-22 §29 → per-post overturning → driven-pile embedment), wired real SolFence vendor data (SKUs / prices / 115 mph / 113 psf), gated the Structural + Mounting Details tabs to drop roof-only UI for fences, added a dedicated SLD fence symbol, fixed the fence BOM to count solar sections from the CAD, and corrected the wind force coefficient to the ASCE value.',
    track: 'engineering-core',
    priority: 'p1',
    status: 'in-progress',
    effort: 'l',
    commits: ['200c6f2b', 'b7fa0722', '5da1dd61', '01ea5df9', '71c57143', '32273780', '06ed9b18', 'b0122caa'],
    files: ['lib/structural-engine-v4.ts', 'lib/bom-system-profiles.ts', 'app/engineering/page.tsx', 'lib/sld-symbols.ts', 'tests/structural-fence-engine.test.ts'],
    notes: 'Roof + UI work shipped to dev. Fence structural output stays ESTIMATE / engineered:false until a licensed PE validates against SolFence stamped load tables (blocked on the vendor stamp from Sarah @ SolFence).',
    createdAt: '2026-06-21',
    updatedAt: '2026-06-22',
  },
  {
    id: 'engine-unification-c3-c8',
    title: 'Engine unification — one structural + one NEC engine of record',
    summary: 'Collapse the divergent engines flagged by the accuracy audit. Structural (C3): V4 is now the SOLE engine of record across tab / autofix / permit / BOM / SLD — V1 and V3 retired. NEC (C8): proved computeSystem and electrical-calc already agree on AC sizing and locked it with a drift-fence test instead of a high-risk refactor. A shared MountContext + an engineered:boolean flag let plumbing ship without un-gating PE geotech.',
    track: 'engineering-core',
    priority: 'p1',
    status: 'in-progress',
    effort: 'xl',
    commits: ['f44cdf8d', '48a1d2be', '85a46930', 'b6808539', '7c0d7eb0'],
    files: ['lib/structural-engine-v4.ts', 'docs/UNIFIED-ENGINE-DESIGN-SPEC.md', 'tests/nec-engine-parity.test.ts'],
    notes: 'Structural C3 substantially complete (V4 everywhere). NEC C8 resolved for accuracy (AC parity fenced; DC parity fence = follow-up). Remaining: PE-gated ground geotech + fence post-overturning (the engineered:true steps).',
    createdAt: '2026-06-20',
    updatedAt: '2026-06-21',
  },
  {
    id: 'engineering-accuracy-audit-2026-06',
    title: 'Engineering pipeline accuracy audit + datasheet verification',
    summary: 'Multi-agent audit of the whole engineering pipeline (sys-config → BOM, including structural / SLD / equipment-schedule). 92 findings, 9 critical. Fixed criticals: BOM battery qty (C6), multi-inverter AC collapse (C7), SLD 120% busbar + battery backfeed (C1/C9), structural corner-zone uplift (C2). Web-verified and corrected real datasheet values across many SKUs (Q CELLS, IQ Battery 10T, Maxeon 3, IQ8, EverVolt, Trina, etc.).',
    track: 'engineering-core',
    priority: 'p0',
    status: 'done',
    effort: 'xl',
    files: ['docs/ENGINEERING-ACCURACY-AUDIT-2026-06-20.md', 'docs/GROUND-FENCE-ACCURACY-AUDIT-2026-06-20.md'],
    notes: 'Root causes: fabricated-defaults-as-fact, no single source of truth, per-brand/topology gaps. Drove the engine-unification + fence + AHJ work. Shipped to dev.',
    createdAt: '2026-06-19',
    updatedAt: '2026-06-21',
  },
  {
    id: 'tesla-certified-integration',
    title: 'Tesla Certified Installer — full Tesla data integration',
    summary: 'Company became a Tesla Certified Installer. Ingesting all Tesla solar product / install / pricing / stringing data and implementing it across the engineering system (catalog, sizing / stringing, structural, BOM, planset) plus a Powershare / whole-home-backup module. Tesla inverter datasheet corrected (4 MPPT, DC/AC 1.7, 17 A/MPPT) and Powerwall + battery-all-brands sizing fixed.',
    track: 'brand-onboarding',
    priority: 'p1',
    status: 'in-progress',
    effort: 'xl',
    commits: ['baf677e7', '4bef0e5f'],
    files: ['_tesla_docs/TESLA-MASTER-SPEC.md', 'lib/equipment-db.ts', 'lib/system/brandProfiles/tesla.ts'],
    notes: 'Source docs extracted to _tesla_docs/. Phases: 0 audit (done), 1 catalog + accuracy (done), 2 stringing temp-adders / MCI, 3 Appendix-C structural, 4 planset, 5 Powershare / Backup Switch. Multi-session. See memory: tesla-integration.',
    createdAt: '2026-06-19',
    updatedAt: '2026-06-22',
  },
];
