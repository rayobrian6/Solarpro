...
// ═══════════════════════════════════════════════════════════════════════════
// RETIRED SECOND THERMAL DATASET — this module no longer owns design
// temperatures.
//
// This file used to carry its own ASHRAE 2005 state table. It disagreed with
// the ASHRAE 2021 table in lib/permit/utils/designTemps.ts for 50 of 51 states
// (up to 16 °C), so the engineering page and the stamped plan set sized NEC
// 690.7(A) maximum series strings at two different temperatures — in Illinois
// a 40.95 V / -0.27 %/°C module was 13 modules in the app and 12 on the sheet.
//
// There is now ONE authority: getThermalDesignBasis in
// lib/permit/utils/designTemps.ts. The function below is a thin delegate to it,
// retained only so existing test importers keep compiling; it defines no values
// of its own. New code must call getThermalDesignBasis directly.
// ═══════════════════════════════════════════════════════════════════════════
/**
 * @deprecated Call `getThermalDesignBasis` from `lib/permit/utils/designTemps`
 * instead. This delegate owns no data; it exists only so that test importers of
 * the retired API keep compiling, and it must be deleted with the last of them.
 *
 * Returns the canonical basis in the retired shape: both fields are °C (the old
 * table stored `maxTemp` in °F and converted on the way out — a unit trap that
 * no longer exists because the value now comes from the authority in °C).
 */
export function getDesignTemperatures(stateCode: string): { minTemp: number; maxTemp: number } {
  const basis = getThermalDesignBasis({ state: stateCode || null });
  return { minTemp: basis.minDesignTempC, maxTemp: basis.maxDesignTempC };
}
