...
export interface ComputedSystemInput {
  ...
  // v47.410 — Optimizer rated max output current (A).
  //   Used only when topology === 'optimizer'. Drives per-string design current
  //   per NEC 690.8(A)(2) instead of the panel-Isc method. When omitted, falls
  //   back to 15.0 A (covers SolarEdge P-series, Tigo TS4-A-O, etc.).
  //   Must be kept consistent with the value passed to `generateStringConfig`
  //   in lib/string-generator.ts so both pipelines produce matching numbers.
...
export function computeSystem(input: ComputedSystemInput): ComputedSystem {
  ...
  if (isString) {
    ...
    // v47.410 — Topology-aware per-string design current (parallel to
    // lib/string-generator.ts). Prior to v47.410, stringIsc here was always
    // panel-Isc × temp-factor regardless of topology, which caused SolarEdge/
    // Tigo optimizer systems to render panel-method ampacity numbers in the
    // SLD, permit PDF, and compliance views — even though optimizer strings
    // are regulated to the optimizer's rated output (typically 15.0 A for
    // P-series). With this change, stringIsc reflects:
    //   'string' → Isc × 1.25 (continuous-duty-adjusted, matches string-generator)
    //   'optimizer' → optimizer rated output cap per NEC 690.8(A)(2)
    ...
    for (let i = 0; i < stringCount; i++) {
      ...
      const iscCorrected = input.panelIsc * iscTempFactor;
      // Topology-aware design current (NEC 690.8(A)(1) vs (A)(2)).
      const stringIsc = isOptimizer
        ? optimizerCap                      // NEC 690.8(A)(2) — regulated output
        : iscCorrected * 1.25;              // NEC 690.8(A)(1) — panel method
      // NEC 690.9(B) OCPD sizing: design-current × 1.25, rounded up to standard.
      const ocpdAmps = nextStandardOCPD(stringIsc * 1.25);

      // v47.414 — Topology-aware voltage-pass gate (per-string validation only;
      // layout math is INTENTIONALLY left untouched in this file to avoid the
      // v47.412 regression where a 200-panel optimizer ceiling produced a
      // single 36-panel string object that downstream React components could
      // not handle, blanking the engineering page).
      //
      // Optimizer systems regulate string voltage at the inverter's DC bus
      // (typically 380–400 V for SolarEdge HD-Wave) regardless of panel
      // count, so panel-Voc × N is inapplicable. Mark voltagePass=true for
      // optimizer topology so the spurious NEC_690_7_VOLTAGE error does not
      // fire in the compliance tab. The existing maxPanelsPerString Voc×N
      // math remains as-is — the sizing engine's brand profile (25 panels
      // max for SolarEdge) enforces the real hardware ceiling upstream.
      const voltagePass = isOptimizer ? true : stringVoc <= input.inverterMaxDcV;
      if (!voltagePass) {
        issues.push({
          severity: 'error',
