// ============================================================================
// v47.436 - Survey Handoff: Token Minter
//
...
//   project_id  - SolarPro project UUID
//
...
export function mintHandoffToken(
  ...
  const ttl = parseInt(process.env.HANDOFF_TOKEN_TTL_SECONDS ?? '600', 10);
  const safeTtl = Number.isFinite(ttl) && ttl > 0 ? ttl : 600;
...
export function buildHandoffUrl(payload: HandoffTokenPayload): string {
  ...
  // Partner mounts the handoff router at /api/handoff/:token (GET).
  // The previous path /survey/<token> was wrong — it pointed at a non-existent
  // SolarPro frontend route instead of the partner's handoff endpoint.
...
// verifyHandoffToken
//
// Verifies and decodes a handoff JWT. Used in tests and future admin tooling.
