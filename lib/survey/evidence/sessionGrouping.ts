...
export function buildProjectSurveyEvidenceHygiene(
  ...
  const rawManifests = orderedSurveys.map(({ survey, files }) => buildSurveyEvidenceManifest({ survey, files, generatedAt }));
  const allEvidence = rawManifests.flatMap((manifest, manifestIndex) => manifest.items.map(item => ({
    item,
    survey: orderedSurveys[manifestIndex].survey,
  })));

  const evidenceDuplicateGroups = buildEvidenceDuplicateGroups(allEvidence);
  const canonicalEvidenceIds = new Set(evidenceDuplicateGroups.map(group => group.canonicalEvidenceId));
  const duplicateEvidenceIds = new Set(
    evidenceDuplicateGroups.flatMap(group => group.evidenceIds.filter(id => id !== group.canonicalEvidenceId)),
  );
  const canonicalItems = allEvidence
    .filter(({ item }) => canonicalEvidenceIds.has(item.evidenceId) || !duplicateEvidenceIds.has(item.evidenceId))
    .map(({ item }) => annotateCanonicalEvidence(item, evidenceDuplicateGroups));

  const sessionGroups = buildSurveySessionGroups(orderedSurveys, rawManifests, evidenceDuplicateGroups, canonicalEvidenceIds);
  const sessionGroup = sessionGroups.find(group => group.surveyIds.length > 1) ?? sessionGroups[0] ?? null;
  const canonicalSurveyId = sessionGroup?.canonicalSurveyId ?? orderedSurveys[0]?.survey.id ?? null;
  const sessions = orderedSurveys.map(({ survey }, index) => {
    const manifest = rawManifests[index];
    const group = sessionGroups.find(candidate => candidate.surveyIds.includes(survey.id));
    const status = group?.statusBySurveyId[survey.id] ?? 'unknown';
    return {
      surveyId: survey.id,
      surveySessionGroupId: group?.surveySessionGroupId ?? stableGroupId('session', [survey.id]),
      ...
      canonicalEvidenceCount: canonicalItems.filter(item => item.surveyId === survey.id).length,
      categoryCoverage: categoriesPresent(manifest.items),
      duplicateReason: group && group.surveyIds.length > 1 ? group.duplicateReason : null,
  ...
  if (repeatedSubmissionGroups.length > 0) {
    const total = repeatedSubmissionGroups.reduce((sum, group) => sum + group.surveyIds.length, 0);
    warnings.push(`${total} survey submissions detected with overlapping evidence. Raw uploads are preserved; canonical coverage uses deduped representatives.`);
  }
  ...
  return {
    ...
    canonicalSurveyId,
    surveySessionDuplicateStatus: sessionGroup && sessionGroup.surveyIds.length > 1 ? 'canonical' : orderedSurveys.length > 0 ? 'unique' : 'unknown',
    ...
    collapsedDuplicateEvidenceCount: Math.max(0, allEvidence.length - canonicalItems.length),
    banner: repeatedSubmissionGroups.length > 0
...
function buildEvidenceDuplicateGroups(evidence: EvidenceWithSurvey[]): EvidenceDuplicateGroup[] {
  ...
  for (const [fingerprint, members] of groups.entries()) {
    ...
    const surveyIds = Array.from(new Set(members.map(member => member.item.surveyId))).sort();
    result.push({
      evidenceDuplicateGroupId: stableGroupId('evidence', [fingerprint, ...ids]),
  ...
  return result.sort((a, b) => a.evidenceDuplicateGroupId.localeCompare(b.evidenceDuplicateGroupId));
}
...
function buildSurveySessionGroups(
  ...
  for (let i = 0; i < surveys.length; i += 1) {
    for (let j = i + 1; j < surveys.length; j += 1) {
      if (areLikelyRepeatedSessions(surveys[i].survey, manifests[i], surveys[j].survey, manifests[j])) {
  ...
  return Array.from(byRoot.values()).map(indices => {
    const groupSurveys = indices.map(index => surveys[index].survey);
    const groupManifests = indices.map(index => manifests[index]);
    const canonicalSurvey = [...groupSurveys].sort((a, b) => compareBestSurveyFirst(a, b, manifests[surveys.findIndex(entry => entry.survey.id === a.id)], manifests[surveys.findIndex(entry => entry.survey.id === b.id)]))[0];
    ...
    for (const survey of groupSurveys) {
      statusBySurveyId[survey.id] = groupSurveys.length === 1
        ? 'unique'
    ...
    const canonicalCount = allItems.filter(item => canonicalEvidenceIds.has(item.evidenceId)).length
      || (groupSurveys.length === 1 ? allItems.length : 0);
    return {
      ...
      duplicateReason: groupSurveys.length > 1
  ...
  }).sort((a, b) => b.surveyIds.length - a.surveyIds.length || a.surveySessionGroupId.localeCompare(b.surveySessionGroupId));
}
...
function buildCanonicalProjectManifest(input: {
  ...
  const warnings = [
    ...new Set([
      ...
      ...requiredMissing.map(category => `Missing required survey evidence category: ${category}`),
    ]),
  ];
  const classifiedItems = input.items.filter(item => item.processingStatus !== 'uploaded').length;
  const completeness = input.items.length === 0
    ? 'missing'
  ...
  return {
    ...
    summary: {
      ...
      confidence: completeness === 'sufficient' ? 'high' : input.items.length > 0 ? 'medium' : 'unknown' as SurveyEvidenceConfidence,
      completeness,
    },
...
function annotateCanonicalEvidence(item: SurveyEvidenceItem, groups: EvidenceDuplicateGroup[]): SurveyEvidenceItem {
  const group = groups.find(candidate => candidate.evidenceIds.includes(item.evidenceId));
...
function areLikelyRepeatedSessions(
  ...
  const photoCountsSimilar = Math.abs(aManifest.items.length - bManifest.items.length) <= Math.max(2, Math.ceil(Math.max(aManifest.items.length, bManifest.items.length) * 0.2));
  const coverageSimilarity = jaccard(categoriesPresent(aManifest.items), categoriesPresent(bManifest.items));
  const sameTechnician = normalizeText(a.inspectorName ?? extractTechnician(a.surveyData)) === normalizeText(b.inspectorName ?? extractTechnician(b.surveyData));
  const closeWindow = Math.abs(timestamp(a.createdAt) - timestamp(b.createdAt)) <= 1000 * 60 * 60 * 24 * 14;
  const sameSource = a.source === b.source;
  return photoCountsSimilar && coverageSimilarity >= 0.5 && (sameTechnician || sameSource || closeWindow);
}

function compareNewestSurveyFirst(a: SurveyFilesBySurveyId, b: SurveyFilesBySurveyId): number {
  return timestamp(b.survey.createdAt) - timestamp(a.survey.createdAt);
}

function compareBestSurveyFirst(a: SiteSurvey, b: SiteSurvey, aManifest?: SurveyEvidenceManifest, bManifest?: SurveyEvidenceManifest): number {
...
function compareBestEvidenceFirst(a: EvidenceWithSurvey, b: EvidenceWithSurvey): number {
  const aScore = evidenceScore(a.item);
  const bScore = evidenceScore(b.item);
  if (aScore !== bScore) return bScore - aScore;
  return timestamp(b.item.captureTimestamp ?? b.survey.createdAt) - timestamp(a.item.captureTimestamp ?? a.survey.createdAt);
}

function surveyScore(survey: SiteSurvey, manifest?: SurveyEvidenceManifest): number {
  const requiredPresent = manifest?.coverage.filter(group => group.required && group.status === 'present').length ?? 0;
...
function evidenceFingerprint(item: SurveyEvidenceItem): string {
  const filename = normalizeFilename(item.filename ?? filenameFromUrl(item.fileUrl) ?? item.blobKey);
  ...
  return [item.projectId ?? 'no_project', filename, urlHint, mime, category, submitted].join('|');
}
...
function categoriesPresent(items: SurveyEvidenceItem[]): SurveyEvidenceCategory[] {
  return Array.from(new Set(items.filter(item => item.category !== 'uncategorized').map(item => item.category))).sort();
}

function jaccard(a: SurveyEvidenceCategory[], b: SurveyEvidenceCategory[]): number {
...
function normalizeFilename(value: string | null | undefined): string {
  ...
  return base.trim().toLowerCase().replace(/^[0-9]{10,}-/, '').replace(/[\s]+/g, '_');
}
