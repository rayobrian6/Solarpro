...
function parseHandwrittenList(
  ...
  const pat = new RegExp(
    ...
    `([1-9][0-9]{0,3}(?:,[0-9]{3})?)` +                  // kWh value: 10-9999 or 1,234 or 12,345
...
// ── P3: Monthly Usage History / Monthly Usage Summary table ────────────────
/**
 * Utilities print a kWh history section under various headers:
 *
 *   CMP / Eversource style (row-per-month):
 *     "Your Monthly Usage Summary (kWh)"
 *     Month  2025  2024  2023
 *     Jan     362   324   298
 *
 *   Ameren Illinois / ComEd style (bar-chart inline numbers):
 *     "Electric Usage History in Kilowatt Hours (kWh)"
 *     "Energy Usage History by Account Basis (kWh)"
 *     OCR produces: "4188 2311 1418 761 318 460 881 901 1660 2296 1509 1354"
 *
 *   Other common headers:
 *     "Electric Usage History (kWh)", "Usage History in kWh",
 *     "Electricity Usage History", "kWh Usage History"
 *
 * Numbers ARE monthly kWh totals -- use them directly.
 * Valid monthly kWh range: 50-5000.
 *
 * TWO sub-modes:
 *   A) Row-per-month  -- CMP/Eversource: "Jan  362  324" (month name + year cols)
 *   B) Inline list   -- Ameren style: 8-13 integers 50-5000 in the lines after header
 */
function parseBarGraphTable(
  ...
  // Recognised section headers (covers CMP, Ameren, ComEd, Eversource, National Grid, etc.)
  const SECTION_HEADER_RE =
    /monthly\s+usage\s+summary|electric(?:ity)?\s+usage\s+history|energy\s+usage\s+history|usage\s+history\s+(?:in\s+)?(?:kilowatt.?hours?|kwh)|kwh\s+usage\s+history|\busage\s+history\b/i;

  let sectionStart = -1;
  for (let i = 0; i < lines.length; i++) {
    if (SECTION_HEADER_RE.test(lines[i])) {
      sectionStart = i;
      log.push(`[bill-parser] P3 usage history header at line ${i}: "${lines[i].trim()}"`);
      break;
    }
  }

  if (sectionStart === -1) {
    log.push('[bill-parser] P3 bar graph: no usage history section header found -- skipping');
    return { months: {}, evidence: [] };
  }

  // ── Mode A: Row-per-month (CMP / Eversource style) ────────────────────────
  ...
  if (Object.keys(months).length >= 3) {
    const c = Object.keys(months).length;
    log.push(`[bill-parser] P3 mode-A row-per-month: ${c} months → ${JSON.stringify(months)}`);
    return { months, evidence };
  }

  // ── Mode B: Inline kWh list (Ameren / ComEd bar-chart label style) ───────────
  // Ameren prints kWh totals as space-separated numbers above/below bar chart.
  // OCR produces: "4188 2311 1418 761 318 460 881 901 1660 2296 1509 1354"
  //
  // v47.179 FIX: Use CHARACTER-BASED scanning on the original text string (like billOcr.ts)
  // instead of line-based scanning. pdfjs-dist joins all page text onto 1-2 giant lines,
  // so lines[sectionStart] after stripping the header keyword still contains ALL numbers
  // from the entire page (account#, dates, dollar amounts) -- massively polluting allNums.
  // Character-based scan: find header in raw text, take only next 2000 chars.
  const headerMatchForB = SECTION_HEADER_RE.exec(text);
  const modeBScanText = headerMatchForB
    ? text.slice(headerMatchForB.index + headerMatchForB[0].length,
                 headerMatchForB.index + headerMatchForB[0].length + 2000)
    : '';

  const allNumsRaw: number[] = [];

  const numMatches = modeBScanText.match(/\b([0-9]{2,4})\b/g) ?? [];
  for (const n of numMatches) {
    const v = parseInt(n, 10);
    if (v >= 50 && v <= 5000 && !(v >= 1900 && v <= 2099)) {  // exclude year labels
      allNumsRaw.push(v);
    }
  }

  // v47.210: Strip Y-axis scale sequences (chart axis labels like 500,1000,1500... or 0,10,20...)
  // These are arithmetic progressions of 3+ consecutive values with a constant positive step.
  // They contaminate Mode B and cause the best-run algorithm to pick scale values instead of kWh.
  const axisSet = new Set<number>();
  for (let i = 0; i < allNumsRaw.length - 2; i++) {
    const step = allNumsRaw[i + 1] - allNumsRaw[i];
    if (step <= 0) continue; // only increasing sequences
    let j = i + 2;
    while (j < allNumsRaw.length && allNumsRaw[j] - allNumsRaw[j - 1] === step) j++;
    if (j - i >= 3) { // 3+ consecutive evenly-spaced ascending values = likely axis
      for (let k = i; k < j; k++) axisSet.add(allNumsRaw[k]);
    }
  }

  const allNums: number[] = allNumsRaw.filter(v => !axisSet.has(v));

  if (axisSet.size > 0) {
    log.push(`[bill-parser] P3 mode-B: filtered ${axisSet.size} Y-axis scale values: [${[...axisSet].sort((a,b)=>a-b).join(', ')}]`);
  }

  log.push(`[bill-parser] P3 mode-B: found ${allNums.length} candidate values (50-5000): [${allNums.slice(0, 15).join(', ')}${allNums.length > 15 ? '...' : ''}]`);

  if (allNums.length >= 8) {
    // Find the longest contiguous run of plausible monthly kWh values (8-13 items)
    // Seasonal-ratio check: max should not exceed 12x min (handles extreme climates)
    let bestRun: number[] = [];
    let bestStart = 0;
    for (let start = 0; start <= allNums.length - 8; start++) {
      for (let len = Math.min(13, allNums.length - start); len >= 8; len--) {
        const run = allNums.slice(start, start + len);
        const minV = Math.min(...run);
        const maxV = Math.max(...run);
        if (maxV > minV * 15) continue; // implausible spread -- skip
    ...
    if (bestRun.length >= 8) {
      log.push(`[bill-parser] P3 mode-B best run: ${bestRun.length} values → [${bestRun.join(', ')}]`);

      // Detect month order from a label line near the section with 6+ month abbreviations
      let monthOrder: MonthKey[] = [...MONTH_ORDER];
      const monthAbbrevLine = modeBScanText.split('\n').find(l => {
        const matchCount = MONTH_ORDER.filter(m => new RegExp(`\\b${m}\\b`, 'i').test(l)).length;
        return matchCount >= 6;
      });
      if (monthAbbrevLine) {
        const detectedOrder: MonthKey[] = [];
        for (const part of monthAbbrevLine.trim().split(/\s+/)) {
          const key = part.slice(0, 3).toLowerCase() as MonthKey;
          if ((MONTH_ORDER as readonly string[]).includes(key) && !detectedOrder.includes(key)) {
            detectedOrder.push(key);
          }
        }
        if (detectedOrder.length >= 6) {
          monthOrder = detectedOrder;
          log.push(`[bill-parser] P3 mode-B month order from label line: ${detectedOrder.join(',')}`);
        }
      }

      // Assign months in order
      const targetCount = Math.min(bestRun.length, 12);
      const usedMonths = monthOrder.slice(0, targetCount);
      const modeB: Partial<Record<MonthKey, number>> = {};
      for (let i = 0; i < usedMonths.length && i < bestRun.length; i++) {
        modeB[usedMonths[i]] = bestRun[i];
        evidence.push(`${usedMonths[i]}: ${bestRun[i]}`);
      }

      const c = Object.keys(modeB).length;
      log.push(`[bill-parser] P3 mode-B: ${c} months assigned → ${JSON.stringify(modeB)}`);
      if (c >= 6) return { months: modeB, evidence };
    }
  }

  const count = Object.keys(months).length;
  if (count === 0) {
    log.push('[bill-parser] P3 bar graph: header found but modes A+B both found no valid monthly data');
    return { months: {}, evidence: [] };
  }

  log.push(`[bill-parser] P3 usage history: ${count} months → ${JSON.stringify(months)}`);
  return { months, evidence };
}
...
function extractCurrentMonth(text: string, log: string[]): number | null {
  const t = normalise(text);
  const patterns = [
    // "Billable Usage (kWh) - Residential (Schedule 1)  4450" — Dominion Energy format
    /billable\s+usage\s*\([^)]*kwh[^)]*\)[^\n]*?([1-9][0-9]{2,4})(?:\s|$)/i,
    ...
    // "Current Usage  362 kWh" — must NOT match "Avg. Daily Usage  23 kWh"
    ...
    // "Net metered usage: 362 kWh" — exclude daily/avg prefixes
    /(?<!(?:daily|avg|average|per[- ]day)\s{0,10})(?:net\s+)?(?:metered\s+)?usage\s*[:\s]+([1-9][0-9]{1,4})\s*kwh/i,
    // "Electric Usage  362 kWh" (no label) — exclude avg daily
    /\belectric(?:ity)?\s+(?!daily\s+)([1-9][0-9]{1,4})\s*kwh/i,
  ];
