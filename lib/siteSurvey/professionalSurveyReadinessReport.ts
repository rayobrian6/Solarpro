...
export type OperatorSurveyReadinessState =
  | "blocked"
  | "review_required"
  | "geometry_ready"
  | "cad_preview_ready";
...
export interface ProfessionalSurveyReadinessReportV1 {
  ...
  summaries: {
    ...
    warnings: string[];
    geometryIntelligence: ReturnType<
      typeof buildOperatorGeometryIntelligenceSummary
    >;
    photoEvidence: ReturnType<typeof buildOperatorPhotoEvidenceSummary>;
    renderIntelligence: ReturnType<
      typeof buildOperatorRenderIntelligenceSummary
    >;
  };
...
export function buildProfessionalSurveyReadinessReport(
  ...
  const cadReadiness = buildSurveyCADReadiness(
    enriched,
    evidence,
    canonicalGeometry,
  );
  const geometryIntelligence = buildGeometryIntelligenceReport({
    evidence,
    canonicalGeometry,
    cadReadiness,
  });
  const photoEvidence = buildSurveyPhotoEvidenceBundle(
    enriched,
    canonicalGeometry,
    photoAnalysis,
  );
  ...
  const renderRecommendationReport =
    buildProfessionalRenderRecommendationReport({
      canonicalGeometry,
      cadReadiness,
      geometryIntelligence,
  ...
  const operatorGeometryIntelligence =
    buildOperatorGeometryIntelligenceSummary(geometryIntelligence);
  const operatorPhotoEvidence =
    buildOperatorPhotoEvidenceSummary(photoEvidence);
  const operatorRenderIntelligence = buildOperatorRenderIntelligenceSummary(
    renderRecommendationReport,
  );
  const readinessState = resolveOperatorReadinessState(
    canonicalGeometry,
    cadReadiness,
  );
  const blockingIssues = dedupe([
    ...evidence.blockingIssues,
    ...canonicalGeometry.blockingIssues,
    ...cadReadiness.blockingIssues,
  ]);
  const warnings = dedupe([
    ...canonicalGeometry.warnings,
    ...cadReadiness.warnings,
  ]);
  const missingRequiredFields = dedupe([
    ...evidence.missingRequiredFields,
    ...cadReadiness.requiredReviewItems,
  ]);
  ...
  return {
    ...
    summaries: {
      ...
      invalidCanonicalRoofPlaneCount: canonicalGeometry.roofPlanes.filter(
        (plane) => !plane.valid,
      ).length,
...
export function siteSurveyToEnrichedSurvey(
  survey: SiteSurvey,
  files: SiteSurveyFile[] = [],
): EnrichedSiteSurvey {
...
function resolveOperatorReadinessState(
  ...
  if (
    cadReadiness.readinessStatus === "blocked" ||
    geometry.blockingIssues.length > 0
  )
    return "blocked";
  if (
    cadReadiness.readinessStatus === "cad_ready" &&
    cadReadiness.canBuildCADInput &&
    cadReadiness.cadInputPreview !== null
  )
    return "cad_preview_ready";
  if (cadReadiness.readinessStatus === "review_required")
    return "review_required";
  if (geometry.readyForCADInput) return "geometry_ready";
  return "review_required";
}

function collectConfidenceGaps(
  evidence: ProfessionalSiteSurveyEvidenceBundleV1,
): string[] {
  const gaps: string[] = [];
  for (const candidate of evidence.roofGeometryCandidates) {
    if (candidate.confidence !== "high")
      gaps.push(
        `roofPlane.${candidate.planeId}: ${candidate.confidence} confidence`,
      );
  }
  for (const candidate of [
    ...evidence.electricalServiceCandidates,
    ...evidence.structuralCandidates,
  ]) {
    if (candidate.confidence === "low")
      gaps.push(`${candidate.field}: low confidence`);
  }
  return dedupe(gaps);
}
