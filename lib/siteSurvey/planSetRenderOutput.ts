...
function renderEvidenceSheet(ctx: ReturnType<typeof buildContext>): PlanSetRenderSheetV1 {
  const r = ctx.report;
  const photos = r.photoEvidence.evidence.slice(0, 10).map((p, i) => photoTile(72 + (i % 2) * 390, 150 + Math.floor(i / 2) * 104, p.source.slotKey, p.classification.category, p.classification.confidence, photoThumbnailForEvidence(ctx, p.source.url), ocrSnippetsForEvidence(ctx, p.source.url))).join('');
...
function photoTile(x: number, y: number, slot: string, cat: string, conf: string, thumbnailDataUrl: string | null = null, ocrSnippets: string[] = []) { const thumb = thumbnailDataUrl ? `<image href="${esc(thumbnailDataUrl)}" x="${x + 12}" y="${y + 12}" width="62" height="46" preserveAspectRatio="xMidYMid meet"/><rect x="${x + 12}" y="${y + 12}" width="62" height="46" fill="none" stroke="${STYLE.grid}"/>` : `<rect x="${x + 12}" y="${y + 12}" width="62" height="46" fill="${STYLE.photo}" stroke="${STYLE.grid}"/><line x1="${x + 12}" y1="${y + 12}" x2="${x + 74}" y2="${y + 58}" stroke="${STYLE.grid}"/><line x1="${x + 74}" y1="${y + 12}" x2="${x + 12}" y2="${y + 58}" stroke="${STYLE.grid}"/>`; const ocr = ocrSnippets.length ? `<text x="${x + 90}" y="${y + 80}" class="tinyStrong" fill="${STYLE.review}">OCR REVIEW-ONLY:</text>${ocrSnippets.slice(0, 2).map((snippet, idx) => `<text x="${x + 174}" y="${y + 80 + idx * 12}" class="tiny" fill="${STYLE.review}">${esc(trunc(snippet, 38))}</text>`).join('')}` : ''; return `<g><rect x="${x}" y="${y}" width="360" height="94" fill="#fff" stroke="${STYLE.ink}" stroke-width="${CAD.hairline}"/>${thumb}<text x="${x + 90}" y="${y + 22}" class="small">${esc(trunc(slot, 30))}</text><text x="${x + 90}" y="${y + 42}" class="tiny">CATEGORY: ${esc(cat)}</text><text x="${x + 90}" y="${y + 60}" class="tiny">PHOTO CONFIDENCE: ${esc(conf)} · ${thumbnailDataUrl ? 'SOURCE THUMBNAIL' : 'SAFE PLACEHOLDER'}</text>${ocr}</g>`; }
