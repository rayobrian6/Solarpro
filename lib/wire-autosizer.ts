...
export function autoSizeACWire(input: WireAutoSizerInput): WireAutoSizerResult {
  ...
  for (let i = searchStart; i < AWG_ORDER.length; i++) {
    ...
    // Voltage drop check
    // FIX v57.1: Guard NaN/Infinity in VDrop — caused by wireLength=0/NaN or systemVoltage=0.
    // NaN <= limit is always false, forcing endless BUMPED_VDROP through all gauges to #2/0 fallback.
    const vdropRaw = calcVoltageDrop(
  ...
  const finalVdropRaw = calcVoltageDrop(
    input.inverterMaxACOutputCurrent,
    input.onewayLengthFt,
    selectedGauge,
    input.systemVoltage
  );
  // FIX v57.1: Guard NaN/Infinity in final AC VDrop
  const finalVdrop = Number.isFinite(finalVdropRaw) ? finalVdropRaw : 0;
...
export function autoSizeDCWire(input: DCWireAutoSizerInput): DCWireAutoSizerResult {
  ...
  for (let i = searchStart; i < AWG_ORDER.length; i++) {
    ...
    // FIX v57.1: Guard NaN/Infinity in DC VDrop
    const vdropRawDC = calcVoltageDrop(
      input.maxCurrentNEC / 1.25, // use Isc_corrected (not × 1.25) for VD calc
      input.onewayLengthFt,
      gauge,
      input.systemVoltage
    );
    const vdrop = Number.isFinite(vdropRawDC) ? vdropRawDC : 0;
    const vdropPass = vdrop <= input.maxVoltageDropPct;
  ...
  const finalAmpacity = finalCond
    ...
    : 0;
  const finalVdropRawDC = calcVoltageDrop(
    input.maxCurrentNEC / 1.25,
    input.onewayLengthFt,
    selectedGauge,
    input.systemVoltage
  );
  // FIX v57.1: Guard NaN/Infinity in DC final VDrop
  const finalVdrop = Number.isFinite(finalVdropRawDC) ? finalVdropRawDC : 0;
