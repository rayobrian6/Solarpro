...
export function getAllUnifiedPanels(libPanels: LibPanel[] = []): UnifiedPanel[] {
  ...
  // engMap lets library panels pull engineering specs when IDs match
  const engMap = new Map(ENG_PANELS.map(p => [p.id, p]));
  const result: UnifiedPanel[] = [];
  const seenIds = new Set<string>();

  // First: library panels (user library is panels[0], so default panel is correct)
  // Library panels whose IDs match engineering DB get engineering specs merged in.
  for (const lp of libPanels) {
    const ep = engMap.get(lp.id);
    if (ep) {
      // Matching ID: use engineering electrical specs + library dimensions/pricing
      result.push(engPanelToUnified(ep, lp));
    } else {
      // Library-only (custom) panel
      result.push(libPanelToUnified(lp));
    }
    seenIds.add(lp.id);
  }

  // Second: engineering panels not already covered by library
  for (const ep of ENG_PANELS) {
    if (!seenIds.has(ep.id)) {
      result.push(engPanelToUnified(ep, undefined));
      seenIds.add(ep.id);
    }
  }
