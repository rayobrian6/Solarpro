    ...
    incentives: [
      {
        ...
        valueType: 'per_kwh', value: 0.006,
        residential: true, commercial: true,
        description: 'Ohio SRECs tradeable in PJM/regional market. 1 SREC = 1 MWh. Market price ~$5-8/SREC ($0.005-0.008/kWh). Value fluctuates with supply/demand.',
        administrator: 'Ohio PUC / SRECTrade', stackable: true,
      },
      {
...
export function calculateIncentives(
  ...
  // Federal ITC
  // P.L. 119-21 (One Big Beautiful Bill Act, July 4 2025): residential ITC (§25D) repealed
  // for installs completed after Dec 31 2025. Commercial ITC (§48E) still 30%.
  // We conservatively apply 0% for residential, 30% for commercial.
  const federalItcRate = isResidential ? 0 : 30;
  const federalValue = Math.round(systemCost * (federalItcRate / 100));
  const federal: IncentiveCalculation = {
    ...
    description: isResidential
      ? 'Residential ITC (§25D) repealed by P.L. 119-21 for 2026+ installs — $0'
      : `${federalItcRate}% × $${systemCost.toLocaleString()} system cost (Commercial §48E)`,
  };
