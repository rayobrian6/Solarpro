...
function operationalPayload(
  ...
  return {
    ...
    funnel_slug:
      opts.funnel_slug ?? cleanString(raw.funnel_slug) ?? "free-solar-estimate",
    source_system:
      opts.source_system ??
      normalized?.source_system ??
      cleanString(raw.source_system) ??
      "homeowner_form",
    source_channel:
      normalized?.source_channel ??
      opts.source_channel ??
      cleanString(raw.source_channel) ??
      "web",
    ...
    address_line1:
      ...
      cleanString(raw.address_line1) ??
      cleanString(raw.property_address),
    ...
    homeowner_status:
      cleanString(raw.homeowner_status) ??
      normalized?.home_ownership ??
      cleanString(raw.home_ownership),
    home_ownership:
      normalized?.home_ownership ??
      cleanString(raw.home_ownership) ??
      cleanString(raw.homeowner_status),
    ...
    consent_timestamp:
      normalized?.consent_timestamp ?? cleanString(raw.consent_timestamp),
...
export async function submitHomeownerIntakeEvent(
  rawPayload: Record<string, unknown>,
  options: HomeownerIntakeOptions = {},
): Promise<HomeownerIntakeResult> {
  const startedAt = Date.now();
  const sourceSystem =
    options.source_system ||
    cleanString(rawPayload.source_system) ||
    "homeowner_form";
  const sourceChannel =
  ...
  console.info(
    ...
    safeIntakeLogPayload(rawPayload, null, eventId),
  );
  console.info(
    "[HOMEOWNER PAYLOAD]",
    safeIntakeLogPayload(rawPayload, null, eventId),
  ...
  console.info(
    "[HOMEOWNER INTAKE PAYLOAD]",
    safeIntakeLogPayload(rawPayload, null, eventId),
  );
  console.info(
    "[INTAKE NORMALIZATION]",
    safeIntakeLogPayload(payload, normalized, eventId),
  );
  console.info(
    "[HOMEOWNER INTAKE NORMALIZED]",
    safeIntakeLogPayload(payload, normalized, eventId),
  );
  ...
  const attribution = payload.attribution as
    | Record<string, unknown>
    | undefined;
  ...
  if (!validation.valid || !normalized) {
    ...
    console.info("[INTAKE EVENT PERSISTED]", {
      event_id: eventId,
    ...
    console.info("[HOMEOWNER INTAKE PERSISTED]", {
      event_id: eventId,
      event_type: "homeowner_intake",
      action: "validation_failed",
      review_status: "not_reviewable",
  ...
  try {
    await insertHomeownerEvent({
      ...
      duplicate_result: {
        status: "not_run",
        reason: "review_first_homeowner_event",
      },
      pipeline_result: {
        action: "pending_review",
    ...
    console.error(
      "[homeownerEventIntake] Failed to persist homeowner intake event:",
      err,
    );
    return {
      action: "error",
...
export async function logMalformedHomeownerIntake(
  message: string,
  options: HomeownerIntakeOptions = {},
): Promise<string | null> {
  ...
  try {
    ...
    console.warn(
      "[homeownerEventIntake] Failed to log malformed homeowner intake:",
