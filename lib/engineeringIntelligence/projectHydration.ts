...
import {
  buildCADReadinessMetadata,
  type CADReadinessMetadataModel,
} from "./cadReadiness";
import {
  buildDeterministicPhotoGrouping,
  type DeterministicPhotoGroupingModel,
} from "./photoGrouping";
import {
  computeEngineeringInvalidationPropagation,
  type EngineeringInvalidationPropagationResult,
} from "./invalidationEngine";
import {
  buildEngineeringRegenerationPlanV1,
  type EngineeringRegenerationPlanV1,
} from "./regenerationPlanner";
import {
  buildEngineeringSnapshotDeltaV1,
  type EngineeringSnapshotDeltaV1,
} from "./snapshotDelta";
...
import type { EngineeringContextResolutionSummary } from "./contextTypes";
...
} from "@/lib/survey/evidence/categoryRegistry";
...
export async function hydrateProjectEngineeringIntelligenceFromDb(input: {
  ...
  try {
    const surveys = await getSiteSurveysByProject(
      input.projectId,
      input.userId,
    );
    const sources = await Promise.all(
      surveys.map(async (survey) => ({
        survey,
        files: await getSiteSurveyFiles(survey.id),
      })),
    );
    ...
    const cadReadiness = buildCADReadinessMetadata({
      projectId: input.projectId,
    });
    ...
    const signalAwareCADReadiness = buildCADReadinessMetadata({
      projectId: input.projectId,
      structuredSignals,
    });
    const contextResolution = buildEngineeringContextResolution({
      projectId: input.projectId,
      structuredSignals,
      photoGrouping,
    ...
    const contextAwareCADReadiness = buildCADReadinessMetadata({
      projectId: input.projectId,
      structuredSignals,
      contextResolution,
    });
...
export function hydrateProjectEngineeringIntelligence(input: {
  ...
  const sortedSources = [...input.sources].sort((a, b) => {
    const date = (b.survey.createdAt ?? "").localeCompare(
      a.survey.createdAt ?? "",
    );
  ...
  const hygiene = buildProjectSurveyEvidenceHygiene({
    projectId: input.projectId,
    surveys: sortedSources,
    generatedAt,
  });
  const canonicalSurveyId =
    hygiene.canonicalSurveyId ?? sortedSources[0].survey.id;
  const canonicalSource =
    sortedSources.find((source) => source.survey.id === canonicalSurveyId) ??
    sortedSources[0];
  const canonicalManifest = buildSurveyEvidenceManifest({
    survey: canonicalSource.survey,
    files: canonicalSource.files,
    generatedAt,
  });
  const surveyEvidence = collectEngineeringSurveyEvidence(
    surveyToEnriched(canonicalSource.survey, canonicalSource.files),
    {
  ...
  const bomMetadata = decisionProvenance
    ? buildDecisionAwareBOMMetadata({
        bomItems: [],
        decisionBundle: decisionProvenance,
      })
    : [];
  const sldMetadata = decisionProvenance
    ? buildDecisionAwareSLDMetadata({ decisionBundle: decisionProvenance })
    : null;
  ...
  const updatedRegistry = invalidationResult
    ? {
        ...registry,
        stateRecords: invalidationResult.updatedStateRecords,
        stateIds: invalidationResult.updatedStateRecords
  ...
  const diffs = [
    diffEngineeringStateSnapshots(baselineSnapshot, latestSnapshot),
  ];
  ...
  const baseCADReadiness = buildCADReadinessMetadata({
    projectId: input.projectId,
  ...
  const signalAwareCADReadiness = buildCADReadinessMetadata({
    ...
    canonicalManifest: hygiene.canonicalManifest ?? canonicalManifest,
    surveyEvidence,
    structuredSignals: preliminarySignals,
  });
...
function selectInvalidationEvidenceId(
  evidence: EngineeringSurveyEvidence,
): string | null {
  const electrical = evidence.requirementEvaluation.allRequirements.find(
    (requirement) => requirement.requirementId === "main_service_panel",
  )?.canonicalEvidenceIds[0];
  const utility = evidence.requirementEvaluation.allRequirements.find(
    (requirement) => requirement.requirementId === "utility_meter",
  )?.canonicalEvidenceIds[0];
  const roof = evidence.requirementEvaluation.allRequirements.find(
    (requirement) => requirement.requirementId === "roof_overview",
  )?.canonicalEvidenceIds[0];
  return electrical ?? utility ?? roof ?? evidence.photos[0]?.id ?? null;
}

function surveyToEnriched(
  survey: SiteSurvey,
  files: SiteSurveyFile[],
): EnrichedSiteSurvey {
...
function rawLocation(
  ...
  return {
    ...
    address:
      ...
      stringValue(siteOverview.siteAddress) ??
      null,
  };
}
...
function rawGeometry(
  data: Record<string, unknown>,
): RawSurveyPayload["geometry"] {
  const geometry = record(data.geometry);
  const obstructions = record(data.obstructions);
  return {
    roofPlanes: arrayValue(geometry.roofPlanes),
    obstructions:
      arrayValue(geometry.obstructions) ??
      arrayValue(obstructions.obstructions),
    setbacks: arrayValue(geometry.setbacks),
    usableAreaSqFt: numberOrNull(geometry.usableAreaSqFt),
  };
}

function rawStructural(
  data: Record<string, unknown>,
): RawSurveyPayload["structural"] {
  const structural = record(data.structural);
  const roofConditions = record(data.roofConditions);
  return {
    rafterSpacingIn: scalarValue(
      structural.rafterSpacingIn ?? roofConditions.rafterSpacing,
    ),
    rafterSize: stringValue(structural.rafterSize ?? roofConditions.rafterSize),
    deckingThicknessIn: scalarValue(
      structural.deckingThicknessIn ?? roofConditions.deckingThicknessIn,
    ),
    windExposure: stringValue(structural.windExposure),
    snowLoadPsf: numberOrNull(structural.snowLoadPsf),
    roofCondition: stringValue(
      structural.roofCondition ?? roofConditions.roofCondition,
    ),
    roofAgeYears: numberOrNull(
      structural.roofAgeYears ?? roofConditions.roofAgeYears,
    ),
    atticAccess: booleanOrNull(
      structural.atticAccess ?? roofConditions.atticAccess,
    ),
    roofMaterial:
      stringValue(structural.roofMaterial) ??
      stringValue(roofConditions.roofMaterial),
    roofPitch: stringValue(structural.roofPitch ?? roofConditions.roofPitch),
    stories:
      stringValue(structural.stories) ?? stringValue(roofConditions.stories),
    structureType:
      stringValue(structural.structureType) ??
      stringValue(roofConditions.structureType),
  };
}

function rawElectrical(
  data: Record<string, unknown>,
): RawSurveyPayload["electrical"] {
  const electrical = record(data.electrical);
  const electricalService = record(data.electricalService);
  return {
    mainPanelRatingAmps: scalarValue(
      electrical.mainPanelRatingAmps ??
        electricalService.mainPanelRatingAmps ??
    ...
    breakerSpacesAvailable: scalarValue(
      electrical.breakerSpacesAvailable ??
        electricalService.breakerSpacesAvailable,
    ),
    serviceEntrance: stringValue(
      electrical.serviceEntrance ?? electricalService.serviceEntrance,
    ),
    meterType: stringValue(
      electrical.meterType ?? electricalService.meterSocketType,
    ),
    interconnectionPoint: stringValue(
      electrical.interconnectionPoint ?? electricalService.interconnectionPoint,
    ),
    panelBrand: stringValue(
      electrical.panelBrand ?? electricalService.panelBrand,
    ),
    hasSubPanel: booleanOrNull(
      electrical.hasSubPanel ?? electricalService.hasSubPanel,
    ),
    subPanelRatingAmps: scalarValue(
      electrical.subPanelRatingAmps ?? electricalService.subPanelRating,
    ),
    availableBreakerSlots:
      stringValue(electrical.availableBreakerSlots) ??
      stringValue(electricalService.availableBreakerSlots),
  };
}
...
function surveyPhotoCategory(label: string | null): SurveyPhotoRef["category"] {
  const canonical = normalizeSurveyEvidenceCategory(label);
  const inferred =
    canonical === "uncategorized"
      ? inferSurveyEvidenceCategoryFromText(label)
      : canonical;

  if (
    inferred === "main_service_panel" ||
    inferred === "subpanel" ||
    inferred === "disconnect" ||
    inferred === "inverter_location" ||
    inferred === "gateway_location" ||
    inferred === "battery_location"
  )
    return "panel";
  if (
    inferred === "meter" ||
    inferred === "utility_connection" ||
    inferred === "utility_access"
  )
    return "meter";
  if (
    inferred === "roof_plane" ||
    ...
    inferred === "attic" ||
    inferred === "rafters" ||
    inferred === "roof_surface" ||
    inferred === "attic_access"
  )
    return "roof";
  if (inferred === "obstructions") return "obstruction";
  if (
    inferred === "overview" ||
    inferred === "detached_structures" ||
    inferred === "trench_path" ||
    inferred === "garage_interior_wall"
  )
    return "site";
  ...
  if (
    ...
    normalized.includes("rafter") ||
    normalized.includes("attic")
  )
    return "roof";
  if (
    normalized.includes("panel") ||
    normalized.includes("msp") ||
    normalized.includes("disconnect") ||
    normalized.includes("subpanel")
  )
    return "panel";
  if (normalized.includes("meter") || normalized.includes("utility"))
    return "meter";
  if (
    normalized.includes("obstruction") ||
    normalized.includes("vent") ||
    normalized.includes("chimney") ||
    normalized.includes("skylight")
  )
    return "obstruction";
  if (
