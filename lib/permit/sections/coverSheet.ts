...
export function pageCoverSheet(input: PermitInput, cad: CADModel, pageNum: number, totalPages: number): string {
  ...
  const snowPsf       = isGroundSnowEstablished(_spCover as never)
    ? (_spCover.groundSnowPsf ?? project.ahjGroundSnowPsf ?? project.groundSnowPsf ?? '')
    : ENV_NOT_ESTABLISHED;
  ...
  const designRows = [
    ...
    infoRow('GROUND SNOW LOAD',   snowPsf === ENV_NOT_ESTABLISHED ? ENV_NOT_ESTABLISHED
      : snowPsf !== '' ? `${Number(Number(snowPsf).toFixed(1))} PSF` : ''),
      ...
      <div class="col-stack">
        ...
        <div class="sec">
          ...
          <div class="sec-body">
            ...
            ${/* SolarPro PRODUCT POLICY, not a legal claim. This said "PE STAMP REQUIRED FOR PERMIT SUBMISSION PER AHJ" — a hardcoded string with zero jurisdiction input, zero state-licensing input, no project thresholds and no exemptions, rendering on every package. The renderer was the ONLY source of a material legal claim. Ray's ruling 2026-08-30: SolarPro itself requires professional review before PERMIT_READY; that is NOT evidence the AHJ or state requires a PE, a seal, or any particular discipline. Those are separate governed requirements and remain unverified. */ ''}
