...
export function buildPermitDesignSnapshot(
  ...
  const _permitReadiness: {
    ...
    const META: Record<string, { severity: 'blocking' | 'warning'; authorityPath: string; sheets: string[]; resolution: string }> = {
      ...
      'CODE-AUTHORITY-INCOMPLETE': { severity: 'blocking', authorityPath: 'codeAuthority.editions', sheets: ['PV-0', 'PE-1'], resolution: 'Resolve the project jurisdiction — a state is enough to establish the NEC baseline. Archiving the AHJ adoption ordinance upgrades it; no edition is ever inferred.' },
      'CODE-AUTHORITY-CONFLICT': { severity: 'blocking', authorityPath: 'codeAuthority.editions.nec.conflictingClaims', sheets: ['PV-0', 'PE-1'], resolution: 'Two governed ordinances disagree. Determine which governs and archive it; the sheets print the state adoption as the stated basis meanwhile and disclose both claims.' },
      ...
      'PROJECT-AUTHORITY-UNVERIFIED': { severity: 'blocking', authorityPath: 'projectAuthority', sheets: ['PV-0'], resolution: 'Verify address / APN / municipal boundary / AHJ / fire authority via the document registry (no postal inference).' },
      ...
      'DESIGNER-OF-RECORD-MISSING': { severity: 'blocking', authorityPath: 'project.designer', sheets: ['PV-0'], resolution: 'Assign the designer / engineer-of-record before issue.' },
      'ENGINEERING-REVIEW-PENDING': { severity: 'blocking', authorityPath: 'certification.engineeringReviewApproved', sheets: ['PE-1'], resolution: 'Obtain an approved engineering-review record covering the current snapshot digest (D-6).' },
    };
    // 2026-08-29 - 'CERT' REMOVED from every affectedSheets list. RS-1's SHEETS
    // column routed a reviewer to a sheet the package does not contain: the
    // certification content merged into PE-1, and the twenty-sheet index has no
    // CERT entry. A pointer to a sheet that is not there is the same defect as
    // the review record that pointed at a screen nobody built.
    const STRUCT_DEFAULT = { authorityPath: 'structural (structural-engine-v4 objects)', sheets: ['PV-4C', 'PV-3', 'PE-1'], resolution: 'Establish the verified structural authority (capacity / framing / fastener / assembly) before a structural PASS.' };
