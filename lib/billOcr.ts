...
function extractKwh(text: string): { monthly?: number; annual?: number; monthlyHistory?: number[] } {
  ...
  if (hwCount >= 6) {
    ...
    if (isLikelyDailyAvg && inlineCount < 6) {
      // Convert daily averages → monthly totals (CMP/Eversource style)
      ...
      sourceLabel = 'bar-graph-daily-avg';
      console.log('[bill-upload] Bar graph daily-avg → monthly totals:', JSON.stringify(converted));
    } else {
      // Already monthly totals (Ameren / inline style)
      chosenMonths = barGraphMonths;
      sourceLabel = 'bar-graph-monthly';
      console.log('[bill-upload] Bar graph inline monthly totals:', JSON.stringify(barGraphMonths));
    }
  } else if (hwCount > 0) {
    chosenMonths = handwrittenMonths;
...
function extractCustomerName(text: string): string | undefined {
  ...
  const SKIP_WORDS = /^(?:ACCOUNT|SERVICE|INVOICE|AMOUNT|DATE|BALANCE|TOTAL|PAYMENT|PLEASE|CENTRAL|POWER|ENERGY|ELECTRIC|YOUR|PAGE|AND|FOR|THE|NEW|OLD|CURRENT|PRIOR|NON|CMP|AVANGRID|AMEREN|ILLINOIS|MISSOURI|INDIANA|COMED|EVERSOURCE|DOMINION|NATIONAL|GRID|STATEMENT|BILLING|IMPORTANT|INFORMATION|START|YEAR|WITH|SAVINGS|CHECK|SUMMARY|HISTORY|USAGE|CURRENT|PREVIOUS|AVERAGE)$/;
  const lines = text.split('\n');
  for (const line of lines) {
...
export async function parseBillTextWithLLM(text: string): Promise<BillExtractResult> {
  ...
  try {
    ...
    // v47.177: use SUM (not avg*12) when full year available; filter zeros first
    const _hist2 = merged.monthlyUsageHistory?.filter((v: number) => v > 0) ?? [];
    merged.estimatedAnnualKwh = merged.annualKwh ||
      (_hist2.length >= 10
        ? _hist2.reduce((a: number, b: number) => a + b, 0)
        : _hist2.length >= 3
          ? Math.round(_hist2.reduce((a: number, b: number) => a + b, 0) / _hist2.length * 12)
    ...
    console.warn('[billOcr] LLM fallback failed:', (err as Error).message);
    return regexResult;
  }
}
...
{
  "utility": "string — utility company name (e.g. 'Central Maine Power', 'Eversource', 'Ameren Illinois')",
  ...
  "kwh_usage": "number — electricity usage in kWh for THIS billing period (most recent month only)",
  "annual_kwh": "number — annual or 12-month total kWh if explicitly shown",
  "monthly_kwh_history": "array of numbers — up to 12 monthly kWh values from usage history chart/table, oldest to newest. CRITICAL: extract every individual number from bar charts, graphs, or tables showing month-by-month usage. E.g. [4188, 2311, 1418, 761, 318, 460, 881, 901, 1660, 2296, 1509, 1354]",
...
- For monthly_kwh_history: look for bar charts, usage history tables, or any section showing multiple months of kWh data. Extract ALL individual monthly values as an array.
...
${sanitizeForPrompt(ocrText.substring(0, 4000))}`,
    ...
    const mergedHistory = (existingResult.monthlyUsageHistory && existingResult.monthlyUsageHistory.filter(v => v > 0).length >= 6)
      ? existingResult.monthlyUsageHistory   // Keep good existing history
      : (aiMonthlyHistory ?? existingResult.monthlyUsageHistory);  // Use AI history if we don't have good existing data
    ...
    const merged: BillExtractResult = {
      ...
      monthlyUsageHistory:  mergedHistory,
    ...
    // v47.177: use SUM (not avg*12) when full year available to avoid undercount from zero-padded arrays
    const _hist = merged.monthlyUsageHistory?.filter((v: number) => v > 0) ?? [];
    merged.estimatedAnnualKwh =
      merged.annualKwh ??
      (_hist.length >= 10
        ? _hist.reduce((a: number, b: number) => a + b, 0)   // Full year: direct sum
        : _hist.length >= 3
          ? Math.round(_hist.reduce((a: number, b: number) => a + b, 0) / _hist.length * 12)  // Partial: avg*12
          : merged.monthlyKwh ? merged.monthlyKwh * 12
          : existingResult.estimatedAnnualKwh);
