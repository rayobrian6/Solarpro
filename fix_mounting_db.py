#!/usr/bin/env python3
"""
Fix mounting-hardware-db.ts:
1. Remove the incorrectly-schemed entries added by expand_mounting_db.py
2. Add correct entries matching the exact MountingSystemSpec interface
"""

with open('lib/mounting-hardware-db.ts', 'r') as f:
    content = f.read()

# Find and remove everything from the bad insertion point to the closing ];
# The bad entries start with the comment "// ─── ADDITIONAL RESIDENTIAL RACKING"
BAD_START = '\n  // ─── ADDITIONAL RESIDENTIAL RACKING ───────────────────────────────────────\n'
BAD_END = '];\n\n// ─'

start_idx = content.find(BAD_START)
if start_idx == -1:
    print("Bad entries not found — may already be clean")
else:
    end_idx = content.find(BAD_END, start_idx)
    if end_idx == -1:
        print("ERROR: Could not find end of bad entries")
        exit(1)
    # Remove the bad entries
    content = content[:start_idx] + '\n' + content[end_idx:]
    print(f"✅ Removed bad entries (chars {start_idx} to {end_idx})")

# Now insert correct entries before the closing ];
...
CORRECT_SYSTEMS = '''
...

  {
    ...
    mount: {
      model: 'GLIDE Screw Pile',
      attachmentMethod: 'screw_pile',
...
print(f"Inserting before index {idx}")
print(f"Context: {repr(content[idx-80:idx+30])}")

new_content = content[:idx] + CORRECT_SYSTEMS + content[idx:]
...
lines = [l for l in result2.stdout.split('\n') if 'category:' in l and 'SystemCategory' not in l and '//' not in l]
